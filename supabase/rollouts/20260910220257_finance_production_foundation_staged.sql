-- Production rollout, user authorized 2026-09-10. Not part of the fresh-install chain.
-- Installs the original additive ledger foundation, with access held closed until activation.
...
-- No new financial API may be used while dependent migrations are pending.
-- Activation must restore the reviewed can_access definition after the complete rollout.
create or replace function finance_private.can_access(_tenant uuid) returns boolean
language sql stable security definer set search_path='' as $$ select false; $$;
comment on function finance_private.can_access(uuid) is 'Finance rollout staged 20260910220257: access held closed pending dependent migrations and activation.';