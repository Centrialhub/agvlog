-- Original: 20260910012756_finance_fiscal_queue_worker.sql; SHA256: 3d209e4971a591725a5c5e859ebca9f18457e4f3659f656ab1cef8f3a8f31549
...
begin
 ...
 if exists(select 1 from cron.job where jobname='finance-fiscal-projection-every-minute') then raise exception 'finance_staged_job_already_exists';end if;
 execute $original_worker$alter table public.finance_fiscal_projection_jobs
...
$original_worker$;
 ...
 where jobname='finance-fiscal-projection-every-minute' and username=current_user and database=current_database();
 ...
 if not exists(select 1 from cron.job where jobid=scheduled_id and active is false
   and schedule='* * * * *' and command='SET statement_timeout = ''25s''; SELECT finance_private.run_fiscal_queue(50);'
   and username=current_user and database=current_database())
 or (select count(*) from cron.job where jobname='finance-fiscal-projection-every-minute')<>1 then
