-- Original: 20260910022059_finance_automatic_reference_reconciliation.sql; SHA256: 3c3f6e1762fe872589ca0732bf97ba76f1535a65bec4454aa0f109ce6333926c
-- STAGED ONLY; do not also apply the original migration to this database.
-- Complete original source is embedded verbatim below. One statement is atomic,
-- including scheduling and deactivation: no committed active-job interval.
do $stage_worker$
...
begin
 ...
 or to_regclass('cron.job') is null then
  raise exception 'finance_staged_cron_dependency_missing';
 end if;
 if exists(select 1 from cron.job where jobname='finance-bank-reconciliation-every-minute') then raise exception 'finance_staged_job_already_exists';end if;
 execute $original_worker$alter table public.finance_reconciliation_groups drop constraint finance_reconciliation_groups_method_check;
...
$original_worker$;
 select jobid into strict scheduled_id from cron.job
 where jobname='finance-bank-reconciliation-every-minute' and username=current_user and database=current_database();
 perform cron.alter_job(scheduled_id, active := false);
 if not exists(select 1 from cron.job where jobid=scheduled_id and active is false
   and schedule='* * * * *' and command='SET statement_timeout = ''25s''; SELECT finance_private.run_automatic_reconciliation_queue();'
   and username=current_user and database=current_database())
 or (select count(*) from cron.job where jobname='finance-bank-reconciliation-every-minute')<>1 then
  raise exception 'finance_staged_pause_not_confirmed';
 end if;
end;
$stage_worker$;