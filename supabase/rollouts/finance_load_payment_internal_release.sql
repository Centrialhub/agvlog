...
-- Finance-only boundary. Original load-payment SQL above remains verbatim.
do $finance_load_boundary$
declare p record; definition text; guarded text;
begin
 if to_regprocedure('finance_private.require_access(uuid)') is null then raise exception 'finance_load_boundary_dependency_missing';end if;
 select r.*,l.lanname into p from pg_proc r join pg_language l on l.oid=r.prolang where r.oid=to_regprocedure('public.apply_load_payment_command(jsonb)');
 if not found then raise exception 'finance_load_boundary_source_missing';end if;
 if md5(replace(p.prosrc,E'\r\n',E'\n'))<>'0609e96484f48a898608c89a7f127ad3' or not p.prosecdef or p.lanname<>'plpgsql' or p.provolatile<>'v'
 or p.proconfig is distinct from array['search_path=""']::text[] or p.proargnames is distinct from array['_payload']::text[]
 ...
 raise exception 'finance_load_boundary_contract_changed';end if;
 definition:=pg_get_functiondef(p.oid);
 guarded:=replace(p.prosrc,'  select * into v_previous',E'  perform finance_private.require_access(v_tenant);\n  select * into v_previous');
 guarded:=E'<<finance_load_entry_guard>>\nBEGIN\n PERFORM finance_private.require_access((_payload->>''tenant_id'')::uuid);\n <<original_load_payment>>\n'||guarded||E'\nEND;\n';
 execute replace(definition,p.prosrc,guarded);
end;$finance_load_boundary$;
