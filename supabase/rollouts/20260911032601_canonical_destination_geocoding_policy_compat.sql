-- Full reviewed211000 with compatibility only for seven existing company policies.
-- Original SHA256 d767b8347432294fda349e199361cb085a7a5b88f5d820713fafea62e52f33cc. No existing policy is dropped by this adapter.
-- The original named-policy replacements below remain unchanged.
do $preserve_company_policies$
declare spec record;policy record;actual_hash text;
begin
 for spec in select * from(values ('address_resolution_queue','agvlog_active_tenant_context','b17f9ea43b12427a037174f2fec3c257'),
 ('geofence_radius_policies','agvlog_active_tenant_context','b17f9ea43b12427a037174f2fec3c257'),
 ('geofences','agvlog_active_tenant_context','b17f9ea43b12427a037174f2fec3c257'),
 ('geofences','agvlog_delete_authenticated','63a56747207803d5b16afcc352a9a62f'),
 ('geofences','agvlog_insert_authenticated','53b2c1a7ae43cdf15e00bab18ba7df19'),
 ('geofences','agvlog_select_authenticated','b985656eee050bef080703660315fa36'),
 ('geofences','agvlog_update_authenticated','8e32f45386bd7b2202be9972afcc61eb')) e(relation,name,hash) loop
  select * into policy from pg_policy where polrelid=to_regclass('public.'||spec.relation) and polname=spec.name;
  if not found then raise exception 'canonical_company_policy_missing: %.%',spec.relation,spec.name;end if;
  actual_hash:=md5(jsonb_build_array(policy.polcmd,policy.polpermissive,
    regexp_replace(replace(coalesce(pg_get_expr(policy.polqual,policy.polrelid),'NULL'),'public.',''),'\s','','g'),
    regexp_replace(replace(coalesce(pg_get_expr(policy.polwithcheck,policy.polrelid),'NULL'),'public.',''),'\s','','g'))::text);
  if actual_hash<>spec.hash or policy.polroles is distinct from array[(select oid from pg_roles where rolname='authenticated')]::oid[]
    or not (select relrowsecurity from pg_class where oid=policy.polrelid) then
   raise exception 'canonical_company_policy_contract_changed: %.%',spec.relation,spec.name;end if;
 end loop;
end;$preserve_company_policies$;
...
begin
  if not (select relrowsecurity from pg_class where oid='public.canonical_addresses'::regclass)
    ...
    or exists(select 1 from pg_policy where polrelid in ('public.canonical_addresses'::regclass,
      ...
      and not ((polname='agvlog_active_tenant_context' and polrelid in('public.geofences'::regclass,'public.geofence_radius_policies'::regclass,'public.address_resolution_queue'::regclass))
        or (polrelid='public.geofences'::regclass and polname in('agvlog_delete_authenticated','agvlog_insert_authenticated','agvlog_select_authenticated','agvlog_update_authenticated')))      and coalesce(pg_get_expr(polqual,polrelid),pg_get_expr(polwithcheck,polrelid),'') not like '%request_tenant_id%')
    or 9<>(select count(*) from pg_policy where polrelid in ('public.canonical_addresses'::regclass,
        'public.operator_command_ledger'::regclass,'public.address_resolution_queue'::regclass,
        'public.geofences'::regclass,'public.geofence_radius_policies'::regclass)

      and not ((polname='agvlog_active_tenant_context' and polrelid in('public.geofences'::regclass,'public.geofence_radius_policies'::regclass,'public.address_resolution_queue'::regclass))
        or (polrelid='public.geofences'::regclass and polname in('agvlog_delete_authenticated','agvlog_insert_authenticated','agvlog_select_authenticated','agvlog_update_authenticated')))      and coalesce(pg_get_expr(polqual,polrelid),pg_get_expr(polwithcheck,polrelid),'') like '%request_tenant_id%')
...
-- Ensure all seven preserved policies remain byte-equivalent semantically.
do $preserve_company_policies$
declare spec record;policy record;actual_hash text;
begin
 for spec in select * from(values ('address_resolution_queue','agvlog_active_tenant_context','b17f9ea43b12427a037174f2fec3c257'),
