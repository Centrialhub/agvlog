-- Production upgrade only: 231003 + audited233637 compatibility + three password-policy helpers.
-- Apply as a single migration transaction; no intermediate MFA-only state is committed.
-- Original231003 and233637 are incorporated by this release; do not reapply them afterwards.
...
-- Scoped forward policy: only three finance helpers from164442; no global role, chat, SSX, data or grants changed.
set local lock_timeout='3s';set local statement_timeout='30s';
do $guard$ begin
