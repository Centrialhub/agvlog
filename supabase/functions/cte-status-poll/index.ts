  ...
  try {
    ...
    for (const doc of (docs || [])) {
      ...
      if(emission.dispatch_key) {
        ...
        if(!outcome && shouldDeadLetter(doc,true))await terminalizeFiscalPoll(admin,{tenantId:doc.tenant_id,documentKind:'cte',documentId:doc.id,documentNumber:doc.invoice_number,reasonCode:'status_timeout',attemptCount:(doc.status_check_attempts||0)+1,firstSeenAt:doc.created_at,context:safeSnapshot});
