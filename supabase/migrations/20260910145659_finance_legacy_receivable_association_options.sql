create function finance_private.legacy_receivable_association(_tenant uuid,_payment uuid,_page integer)
returns jsonb language plpgsql stable security definer set search_path='' as $$
declare payment public.receivables_payments%rowtype;receivable public.receivables%rowtype;
 issue text;received_day date;amount bigint;result jsonb;active_link uuid;
begin
 ...
 select jsonb_build_object('version',1,'tenant_id',_tenant,'payment_id',_payment,'revision',finance_private.legacy_receivable_source_revision(_tenant,_payment),'page',_page,'page_size',20,'total',(select count(*) from candidates),
...
revoke all on function finance_private.legacy_receivable_association(uuid,uuid,integer) from public,anon,authenticated,service_role;
grant execute on function finance_private.legacy_receivable_association(uuid,uuid,integer) to authenticated;
create function public.get_finance_legacy_receivable_association(_tenant_id uuid,_payment_id uuid,_page integer default 1)
returns jsonb language sql stable security invoker set search_path='' as $$select finance_private.legacy_receivable_association(_tenant_id,_payment_id,_page)$$;
revoke all on function public.get_finance_legacy_receivable_association(uuid,uuid,integer) from public,anon,service_role;
grant execute on function public.get_finance_legacy_receivable_association(uuid,uuid,integer) to authenticated;
