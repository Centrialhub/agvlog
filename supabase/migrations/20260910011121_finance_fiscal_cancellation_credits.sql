...
begin
 ...
 body:=replace(body,original,'coalesce(sum(p.amount) filter(where rv.id is null and not exists(select 1 from public.finance_customer_credits credit where credit.tenant_id=p.tenant_id and credit.payment_id=p.id)),0)');
 original:='''reversed_at'',rv.created_at,''reversal_reason'',rv.reason';
 if position(original in body)=0 then raise exception 'finance_credit_history_contract_changed';end if;
 body:=replace(body,original,original||',''credit_id'',(select credit.id from public.finance_customer_credits credit where credit.tenant_id=p.tenant_id and credit.payment_id=p.id)');
 original:='''open_cents'',(greatest(0,r.amount-v_net)*100)::bigint';
