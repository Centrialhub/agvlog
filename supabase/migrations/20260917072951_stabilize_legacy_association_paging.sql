create function finance_private.legacy_payable_catalog_revision(_tenant uuid,_payment uuid) returns text
language plpgsql stable security definer set search_path='' as $$begin return (select md5(jsonb_build_object(
  'source',finance_private.legacy_payable_source_revision(_tenant,_payment),
  'movements',coalesce((select jsonb_agg(jsonb_build_array(m.id,m.amount_cents,finance_private.movement_used_cents(_tenant,m.id)) order by m.id)
   from finance_private.active_movements m join public.payables_payments p on p.tenant_id=m.tenant_id and p.id=_payment
   join public.payables t on t.tenant_id=p.tenant_id and t.id=p.payable_id
   where m.tenant_id=_tenant and m.bank_account_id=p.bank_account_id and m.occurred_on=(p.paid_at at time zone 'America/Sao_Paulo')::date
   and m.direction='out' and m.nature<>'transfer' and(t.driver_id is null or m.driver_id=t.driver_id)),'[]'::jsonb),
  'history',coalesce((select jsonb_agg(jsonb_build_array(to_jsonb(l),to_jsonb(r)) order by l.created_at,l.id)
   from public.finance_payable_movement_links l left join public.finance_payable_link_reversals r on r.tenant_id=l.tenant_id and r.link_id=l.id
   where l.tenant_id=_tenant and l.payment_id=_payment and l.origin='legacy_adoption'),'[]'::jsonb))::text));end$$;
revoke all on function finance_private.legacy_payable_catalog_revision(uuid,uuid) from public,anon,authenticated,service_role;

create function finance_private.legacy_receivable_catalog_revision(_tenant uuid,_payment uuid) returns text
language plpgsql stable security definer set search_path='' as $$begin return (select md5(jsonb_build_object(
  'source',finance_private.legacy_receivable_source_revision(_tenant,_payment),
  'movements',coalesce((select jsonb_agg(jsonb_build_array(m.id,m.amount_cents,finance_private.receipt_movement_used_cents(_tenant,m.id)) order by m.id)
   from finance_private.active_movements m join public.receivables_payments p on p.tenant_id=m.tenant_id and p.id=_payment
   where m.tenant_id=_tenant and m.bank_account_id=p.bank_account_id and m.occurred_on=(p.received_at at time zone 'America/Sao_Paulo')::date
   and m.direction='in' and m.nature in('receipt','customer_advance','other')),'[]'::jsonb),
  'history',coalesce((select jsonb_agg(jsonb_build_array(to_jsonb(l),to_jsonb(r)) order by l.created_at,l.id)
   from public.finance_legacy_receipt_movement_links l left join public.finance_legacy_receipt_link_reversals r on r.tenant_id=l.tenant_id and r.link_id=l.id
   where l.tenant_id=_tenant and l.payment_id=_payment),'[]'::jsonb))::text));end$$;
revoke all on function finance_private.legacy_receivable_catalog_revision(uuid,uuid) from public,anon,authenticated,service_role;

drop function if exists public.get_finance_legacy_payable_association(uuid,uuid,integer);
drop function if exists finance_private.legacy_payable_association(uuid,uuid,integer);
create function finance_private.legacy_payable_association(_tenant uuid,_payment uuid,_page integer,_expected_revision text default null)
...
create function public.get_finance_legacy_payable_association(_tenant_id uuid,_payment_id uuid,_page integer default 1,_expected_revision text default null)
returns jsonb language sql stable security invoker set search_path='' as $$select finance_private.legacy_payable_association(_tenant_id,_payment_id,_page,_expected_revision)$$;
revoke all on function public.get_finance_legacy_payable_association(uuid,uuid,integer,text) from public,anon,authenticated,service_role;
grant execute on function public.get_finance_legacy_payable_association(uuid,uuid,integer,text) to authenticated;

drop function if exists public.get_finance_legacy_receivable_association(uuid,uuid,integer);
drop function if exists finance_private.legacy_receivable_association(uuid,uuid,integer);
create function finance_private.legacy_receivable_association(_tenant uuid,_payment uuid,_page integer,_expected_revision text default null)
returns jsonb language plpgsql stable security definer set search_path='' as $$
declare payment public.receivables_payments%rowtype;receivable public.receivables%rowtype;
 issue text;received_day date;amount bigint;result jsonb;active_link uuid;page_revision text;
begin
 if not finance_private.can_access(_tenant) then raise exception 'finance_access_denied' using errcode='42501';end if;
 if _page is null or _page not between 1 and 100000 or (_expected_revision is not null and _expected_revision!~'^[a-f0-9]{32}$') then raise exception 'finance_invalid_filters' using errcode='22023';end if;
 ...
 if payment.amount is not null and payment.amount::text not in('NaN','Infinity','-Infinity') and payment.amount>0
  and payment.amount*100=trunc(payment.amount*100) and payment.amount*100<=99999999999999 then amount:=(payment.amount*100)::bigint;end if;
 page_revision:=finance_private.legacy_receivable_catalog_revision(_tenant,_payment);
 if _expected_revision is not null and _expected_revision is distinct from page_revision then raise exception 'finance_legacy_association_page_changed' using errcode='40001';end if;
 ...
 with candidates as materialized(
  select m.id,m.bank_account_id,a.name account_name,m.beneficiary_name,m.description,m.occurred_on,m.bank_reference,
   m.amount_cents::text amount_cents,(m.amount_cents-finance_private.receipt_movement_used_cents(_tenant,m.id))::bigint::text remaining_cents
  from finance_private.active_movements m join public.bank_accounts a on a.tenant_id=m.tenant_id and a.id=m.bank_account_id
  where issue is null and m.tenant_id=_tenant and m.bank_account_id=payment.bank_account_id and m.occurred_on=received_day
   and m.direction='in' and m.nature in('receipt','customer_advance','other')
   and m.amount_cents-finance_private.receipt_movement_used_cents(_tenant,m.id)>=amount
 ), page_rows as(select * from candidates order by id limit 20 offset (_page-1)*20),
 history as materialized(
 ...
 select jsonb_build_object('version',1,'tenant_id',_tenant,'payment_id',_payment,'revision',finance_private.legacy_receivable_source_revision(_tenant,_payment),'page_revision',page_revision,'page',_page,'page_size',20,'total',(select count(*) from candidates),
  'rows',coalesce((select jsonb_agg(to_jsonb(p) order by id) from page_rows p),'[]'),
  'payment',jsonb_build_object('id',payment.id,'receivable_id',payment.receivable_id,'bank_account_id',payment.bank_account_id,
   'account_name',(select a.name from public.bank_accounts a where a.tenant_id=_tenant and a.id=payment.bank_account_id),
   'payer_name',(select coalesce(nullif(btrim(to_jsonb(c)->>'company_name'),''),nullif(btrim(to_jsonb(c)->>'name'),'')) from public.clients c where c.tenant_id=_tenant and c.id=receivable.client_id),'received_on',received_day,'amount_cents',amount::text,'bank_transaction_id',payment.bank_transaction_id),
  'eligible',issue is null,'issue',issue,'active_link',active_link,'history_total',(select count(*) from history),
  'history',coalesce((select jsonb_agg(to_jsonb(h) order by created_at desc,id desc) from history_page h),'[]')) into result;
 return result;
end$$;
...
create function public.get_finance_legacy_receivable_association(_tenant_id uuid,_payment_id uuid,_page integer default 1,_expected_revision text default null)
returns jsonb language sql stable security invoker set search_path='' as $$select finance_private.legacy_receivable_association(_tenant_id,_payment_id,_page,_expected_revision)$$;
revoke all on function public.get_finance_legacy_receivable_association(uuid,uuid,integer,text) from public,anon,authenticated,service_role;
grant execute on function public.get_finance_legacy_receivable_association(uuid,uuid,integer,text) to authenticated;
