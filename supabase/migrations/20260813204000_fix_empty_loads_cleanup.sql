-- Fix the trigger and function to ensure empty loads are deleted correctly
-- even when the column change is implicit (like soft delete update)
...
BEGIN
    ...
    -- We must check both status and deleted_at to be safe
