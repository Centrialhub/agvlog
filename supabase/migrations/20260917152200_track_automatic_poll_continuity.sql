...
declare
  ...
  v_automatic_run boolean:=coalesce((_patch->>'automatic_poll_run')::boolean,false);
  v_poll_executed boolean:=coalesce(_patch->'last_run_steps','[]'::jsonb) ? 'position_polling';
  v_automatic_success boolean;
begin
  ...
  v_automatic_success:=_increment_success
    and v_automatic_run
    and coalesce(_patch->>'last_run_mode','') in ('poll','full')
    and v_poll_executed
    and coalesce(_patch->>'last_run_status','')='success';

  if v_automatic_success then
    v_history:=v_previous_health->'recent_automatic_poll_runs';
    ...
    v_health:=v_health||jsonb_build_object(
      'first_automatic_poll_success_at',coalesce(v_previous_health->>'first_automatic_poll_success_at',_patch->>'first_automatic_poll_success_at',v_now::text),
      'last_automatic_poll_success_at',v_now::text,
      'automatic_poll_success_count',coalesce((v_previous_health->>'automatic_poll_success_count')::bigint,0)+1,
      'recent_automatic_poll_runs',v_history
    );
  elsif v_automatic_run then
    v_health:=v_health||jsonb_build_object('last_automatic_poll_failed_at',v_now::text);
  end if;
