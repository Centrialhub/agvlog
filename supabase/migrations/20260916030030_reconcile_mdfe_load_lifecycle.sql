-- Forward reconciliation for environments where the original MDF-e lifecycle migration
-- was present in source control but absent from migration history. Keep this DDL
-- idempotent so an already-reconciled environment remains safe.
