...
begin
  ...
  if not v_route.enabled then
    return jsonb_build_object('version',1,'tenant_id',_tenant_id,'route_id',v_route.id,
      'enabled',false,'archived',true,'revision',v_route.revision);
  end if;
  ...
  return jsonb_build_object('version',1,'tenant_id',_tenant_id,'route_id',v_route.id,
    'enabled',v_route.enabled,'archived',true,'revision',v_route.revision);
end;
$function$;

comment on function public.archive_route_template_v1(uuid,uuid) is
  'Idempotently archives a monitored route, advances its optimistic revision, and preserves waypoints and historical runs.';
