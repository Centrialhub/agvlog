-- Security alignment and explicit RPC grant matrix overhaul

-- 1. Neutralize execute_data_repair_v1 explicitly for all known signatures
...
-- 2. Revoke mass EXECUTE from PUBLIC role for all functions in the public schema
REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC;

-- 3. Inventory and Grant EXECUTE for whitelisted RPCs to the authenticated role
-- We use a DO block to safely grant permissions to specific signatures.
-- This ensures that only these specific function calls are authorized from the frontend.
...
DECLARE
    rpc_list text[] := ARRAY[
        ...
        'list_loads_v1(uuid,text,text[],timestamptz,int)',
...
-- 4. Re-grant EXECUTE to core RLS helpers for authenticated users
-- These are required for data isolation checks to work at runtime.
...
-- 5. Explicitly ensure execute_data_repair_v1 remains blocked for all non-admin roles
REVOKE EXECUTE ON FUNCTION public.execute_data_repair_v1(uuid, uuid) FROM PUBLIC, authenticated, anon;

-- 6. Ensure service_role retains full access for administrative tasks and edge functions
