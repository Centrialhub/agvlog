...
declare
  result jsonb;
  effective_snapshot timestamptz:=coalesce(_snapshot_at,clock_timestamp());
...
begin
  ...
  if _page not between 1 and 1000000
     ...
     or (_from is not null and _from>current_date)
     or (_page>1 and (_snapshot_at is null or _expected_revision is null)) then
  ...
  with source_rows as materialized (
    ...
    where payable.tenant_id=_tenant_id and payable.cost_center is not null
      and payable.created_at<=effective_snapshot
    ...
    where receivable.tenant_id=_tenant_id and receivable.cost_center is not null
      and receivable.created_at<=effective_snapshot
    ...
    where bank_entry.tenant_id=_tenant_id and bank_entry.cost_center is not null
      and bank_entry.posted_at<=effective_snapshot
    ...
    where expense.tenant_id=_tenant_id and expense.cost_center is not null
      and expense.expense_at<=effective_snapshot
    ...
    where maintenance.tenant_id=_tenant_id and maintenance.cost_center is not null
      and maintenance.created_at<=effective_snapshot
    ...
    select md5(coalesce(jsonb_agg(to_jsonb(source_rows) order by occurred_at desc,source_type,id)::text,'[]')) value
    from source_rows
  ), paged as materialized (
    select * from source_rows order by occurred_at desc,source_type,id
    limit _page_size offset (_page-1)*_page_size
  ), by_center as materialized (
    select cost_center name,sum(abs(amount)) value from source_rows where amount<0 group by cost_center
  )
  select revision.value,jsonb_build_object(
    'tenant_id',_tenant_id,'page',_page,'page_size',_page_size,
    'snapshot_at',effective_snapshot,'revision',revision.value,
  ...
  ) into collection_revision,result from revision;

  if _expected_revision is not null and _expected_revision is distinct from collection_revision then
    raise exception 'legacy_cost_center_snapshot_changed' using errcode='40001';
  end if;
  return result;
end;
$function$;

revoke all on function public.get_legacy_cost_center_report_v1(uuid,date,text,integer,integer,timestamptz,text)
from public,anon,authenticated,service_role;
grant execute on function public.get_legacy_cost_center_report_v1(uuid,date,text,integer,integer,timestamptz,text)
to authenticated;
