...
-- 1. Backfill fiscal_documents.recipient_cnpj from clients.tax_id (digits only)
UPDATE public.fiscal_documents fd
SET recipient_cnpj = regexp_replace(c.tax_id, '[^0-9]', '', 'g')
FROM public.clients c
WHERE fd.client_id = c.id
  AND (fd.recipient_cnpj IS NULL OR fd.recipient_cnpj = '')
  AND c.tax_id IS NOT NULL
  AND regexp_replace(c.tax_id, '[^0-9]', '', 'g') <> '';

-- 2. Trigger to keep recipient_cnpj filled on insert/update
CREATE OR REPLACE FUNCTION public.fiscal_documents_autofill_recipient_cnpj()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  v_tax text;
BEGIN
  IF (NEW.recipient_cnpj IS NULL OR NEW.recipient_cnpj = '') AND NEW.client_id IS NOT NULL THEN
    SELECT regexp_replace(tax_id, '[^0-9]', '', 'g') INTO v_tax
    FROM public.clients WHERE id = NEW.client_id;
    IF v_tax IS NOT NULL AND v_tax <> '' THEN
      NEW.recipient_cnpj := v_tax;
    END IF;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_fiscal_documents_autofill_recipient_cnpj ON public.fiscal_documents;
CREATE TRIGGER trg_fiscal_documents_autofill_recipient_cnpj
BEFORE INSERT OR UPDATE OF client_id, recipient_cnpj ON public.fiscal_documents
FOR EACH ROW EXECUTE FUNCTION public.fiscal_documents_autofill_recipient_cnpj();

-- 3. RPC with fallback to clients.tax_id and CNPJ normalization
...
BEGIN
  ...
  -- Recipient with fallback to clients.tax_id
  WITH agg AS (
    SELECT
      fd.recipient,
      COALESCE(
