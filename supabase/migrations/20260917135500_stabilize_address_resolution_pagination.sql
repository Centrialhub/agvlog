drop function if exists public.get_active_address_resolution_queue_v2(uuid,integer,integer);
...
begin
  ...
  with active as materialized (
    ...
    select * from active
    where _cursor_created_at is null or (created_at,id) > (_cursor_created_at,_cursor_id)
    order by created_at,id
    limit _page_limit + 1
  ), visible as materialized (
    select * from candidates order by created_at,id limit _page_limit
  ), continuation as (
    select created_at,id from visible order by created_at desc,id desc limit 1
  )
  select
    coalesce((select jsonb_agg(to_jsonb(v) order by v.created_at,v.id) from visible v),'[]'::jsonb),
    count(*),
    count(*) filter(where status='pending'),
    count(*) filter(where status='ambiguous'),
    count(*) filter(where status='error'),
    v_snapshot_at,
    case when (select count(*) from candidates) > _page_limit then (select created_at from continuation) else null end,
...
comment on function public.get_active_address_resolution_queue_v2(uuid,timestamptz,timestamptz,uuid,integer) is
  'Keyset-pages actionable address resolutions on immutable created_at/id and excludes rows created after the first-page snapshot.';
