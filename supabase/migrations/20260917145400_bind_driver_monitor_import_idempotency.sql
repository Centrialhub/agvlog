...
alter function public.import_driver_monitoring_workbook_v1(jsonb)
  rename to import_driver_monitoring_workbook_unsafe_20260917;
alter function public.import_driver_monitoring_workbook_unsafe_20260917(jsonb)
  set schema private;
revoke all on function private.import_driver_monitoring_workbook_unsafe_20260917(jsonb)
  from public, anon, authenticated, service_role;

create function public.import_driver_monitoring_workbook_v1(_payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if found then
    if v_existing.file_fingerprint is distinct from v_fingerprint
       ...
       or (v_existing.payload_hash is not null and v_existing.payload_hash <> v_payload_hash) then
...
revoke all on function public.import_driver_monitoring_workbook_v1(jsonb)
  from public, anon, authenticated, service_role;
grant execute on function public.import_driver_monitoring_workbook_v1(jsonb)
  to authenticated, service_role;

comment on function public.import_driver_monitoring_workbook_v1(jsonb) is
  'Imports one workbook atomically; replay requires the same request id, file fingerprint, and canonical payload hash.';
