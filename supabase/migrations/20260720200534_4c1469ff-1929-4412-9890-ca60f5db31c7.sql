...
BEGIN
  ...
  PERFORM public._log_entity_audit(
    ...
    'assign_documents',
    NULL,
    jsonb_build_object('document_ids', to_jsonb(_document_ids), 'updated', v_count),
    'composition_rpc'
  );
...
WHERE l.created_at > now() - interval '4 hours'
  AND NOT EXISTS (SELECT 1 FROM public.load_items li WHERE li.load_id = l.id)
  AND NOT EXISTS (SELECT 1 FROM public.fiscal_documents fd WHERE fd.load_id = l.id)
  AND NOT EXISTS (SELECT 1 FROM public.load_orders lo WHERE lo.load_id = l.id)
