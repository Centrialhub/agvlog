-- Final release sweep. This intentionally repeats the idempotent boundary
-- after migrations that may have been developed in parallel with the
...
begin
  ...
  loop
    ...
    execute format('drop policy if exists agvlog_active_tenant_context on %I.%I', relation.schema_name, relation.table_name);
    execute format(
