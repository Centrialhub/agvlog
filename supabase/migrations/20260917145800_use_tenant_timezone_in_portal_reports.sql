...
begin
  ...
  v_start := coalesce(_start_date, v_today - 90);
  v_end := coalesce(_end_date, v_today);
  v_start_at := v_start::timestamp at time zone v_timezone;
  v_end_at := (v_end + 1)::timestamp at time zone v_timezone;
  ...
  document_deliveries as (
    ...
    where f.tenant_id = _tenant_id
      ...
      and coalesce(f.issue_date, timezone(v_timezone, f.created_at)::date)
