create index finance_unloading_origin_command_lookup on public.finance_commands(tenant_id,(result->>'charge_id')) where action='record_unloading';
create function finance_private.unloading_flow_event(_tenant uuid,c jsonb,p jsonb,r jsonb)
returns jsonb language plpgsql stable security definer set search_path='' as $$
declare kind text:=case when r is not null then 'refund' when p is not null then 'receipt' else 'charge' end;
...
begin
 ...
 if kind='charge' then
  ...
  if fc.id is null or fc.action<>'receive' or fc.receivable_id::text<>c->>'receivable_id'
   ...
   or fc.before_snapshot->'evidence'->'receivable'->>'client_id' is distinct from c->>'supplier_id'
  ...
  if fc.before_snapshot->'evidence'->'receivable'->>'amount' ~ '^[0-9]+(\.[0-9]+)?$' and length(fc.before_snapshot->'evidence'->'receivable'->>'amount')<=100 then
   if (fc.before_snapshot->'evidence'->'receivable'->>'amount')::numeric*100 is distinct from (c->>'amount_cents')::numeric then issues:=array_append(issues,'receipt_original_amount_mismatch');end if;
  end if;
  if kind='refund' then
 ...
 return jsonb_build_object('event_id',event_id,'kind',kind,'charge_id',c->'id','receivable_id',c->'receivable_id','supplier_id',c->'supplier_id','supplier_name',c->'source_snapshot'->>'supplier_name','delivery_stop_id',c->'delivery_stop_id',
  ...
  '_proof',jsonb_build_object('charge',c,'payment',p,'refund',r,'origin_command',to_jsonb(cmd),'receipt_command',to_jsonb(fc),'refund_command',to_jsonb(refund_command),'link',to_jsonb(link),'movement',to_jsonb(m),'transaction',to_jsonb(tx),'global_claims',proof));
end$$;
revoke all on function finance_private.unloading_flow_event(uuid,jsonb,jsonb,jsonb) from public,anon,authenticated,service_role;

create function finance_private.unloading_flow_total(_rows jsonb,_kind text,_supplier uuid default null)
returns jsonb language sql immutable set search_path='' as $$
 select jsonb_build_object('count',count(*),'valid',coalesce(bool_and((x->>'valid')::boolean),true),
  'amount_cents',case when coalesce(bool_and((x->>'valid')::boolean),true) then coalesce(sum((x->>'amount_cents')::numeric),0)::text end)
 from jsonb_array_elements(_rows)x where x->>'kind'=_kind and (_supplier is null or x->>'supplier_id'=_supplier::text)
$$;
revoke all on function finance_private.unloading_flow_total(jsonb,text,uuid) from public,anon,authenticated,service_role;

create function finance_private.period_unloading_flow(_tenant uuid,_from date,_to date,_accounts uuid[],_supplier uuid,_page integer,_expected_revision text)
returns jsonb language plpgsql stable security definer set search_path='' as $$
...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object('id',id,'name',name) order by id),'[]') into options from(
  select c.supplier_id id,min(c.source_snapshot->>'supplier_name') name from public.finance_unloading_charges c where c.tenant_id=_tenant group by c.supplier_id
 )s;
 ...
 for source in
  select to_jsonb(c) c,null::jsonb p,null::jsonb r from public.finance_unloading_charges c where c.tenant_id=_tenant and (_supplier is null or c.supplier_id=_supplier)
   and (c.occurred_on is null or not isfinite(c.occurred_on) or c.occurred_on between _from and _to)
  union all select to_jsonb(c),to_jsonb(p),null::jsonb from public.finance_unloading_charges c join public.receivables_payments p on p.tenant_id=c.tenant_id and p.receivable_id=c.receivable_id where c.tenant_id=_tenant and (_supplier is null or c.supplier_id=_supplier)
  ...
  union all select to_jsonb(c),to_jsonb(p),to_jsonb(r) from public.finance_unloading_charges c join public.receivables_payments p on p.tenant_id=c.tenant_id and p.receivable_id=c.receivable_id join public.receivable_payment_reversals r on r.tenant_id=p.tenant_id and r.payment_id=p.id where c.tenant_id=_tenant and (_supplier is null or c.supplier_id=_supplier)
 ...
 select md5(jsonb_build_object('tenant_id',_tenant,'from',_from,'to',_to,'accounts',money->'account_scope','supplier',_supplier,'options',options,'money_revision',money->'revision',
  'events_hash',md5(coalesce(string_agg(md5(x::text),'' order by x->>'kind',x->>'event_id'),'')))::text),count(*) into revision,total from jsonb_array_elements(events)x;
 ...
 select coalesce(jsonb_agg(jsonb_build_object('supplier_id',id,'supplier_name',name,'origin_totals',finance_private.unloading_flow_total(events,'charge',id),
  'receipt_totals',finance_private.unloading_flow_total(events,'receipt',id),'refund_totals',finance_private.unloading_flow_total(events,'refund',id)) order by id),'[]') into groups
 from(select (x->>'supplier_id')::uuid id,min(x->>'supplier_name') name from jsonb_array_elements(events)x group by x->>'supplier_id')g;
 select coalesce(jsonb_agg(x-'_proof_hash'-'_movement_amount' order by x->>'economic_on' desc nulls first,x->>'kind',x->>'event_id'),'[]') into page_rows
 from(select x from jsonb_array_elements(events)x order by x->>'economic_on' desc nulls first,x->>'kind',x->>'event_id' limit 50 offset ((_page::bigint-1)*50))s;
 ...
 return jsonb_build_object('version',1,'tenant_id',_tenant,'period',jsonb_build_object('from',_from,'to',_to),'currency','BRL','timezone','America/Sao_Paulo',
  ...
  'receipt_totals',finance_private.unloading_flow_total(events,'receipt'),'refund_totals',finance_private.unloading_flow_total(events,'refund'),'supplier_groups',groups,'rows',page_rows,'money_links',links,
...
revoke all on function finance_private.period_unloading_flow(uuid,date,date,uuid[],uuid,integer,text) from public,anon,authenticated,service_role;
grant execute on function finance_private.period_unloading_flow(uuid,date,date,uuid[],uuid,integer,text) to authenticated;
create function public.get_finance_period_unloading_flow(_tenant_id uuid,_from date,_to date,_account_ids uuid[],_supplier_id uuid default null,_page integer default 1,_expected_revision text default null)
returns jsonb language sql stable security invoker set search_path='' as $$select finance_private.period_unloading_flow(_tenant_id,_from,_to,_account_ids,_supplier_id,_page,_expected_revision)$$;
revoke all on function public.get_finance_period_unloading_flow(uuid,date,date,uuid[],uuid,integer,text) from public,anon,authenticated,service_role;
grant execute on function public.get_finance_period_unloading_flow(uuid,date,date,uuid[],uuid,integer,text) to authenticated;
