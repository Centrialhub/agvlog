...
declare v_tenant uuid:=nullif(_payload->>'tenant_id','')::uuid;v_request uuid:=nullif(_payload->>'request_id','')::uuid;
  ...
  v_previous_lat double precision:=nullif(_payload->>'previous_lat','')::double precision;
  v_previous_lng double precision:=nullif(_payload->>'previous_lng','')::double precision;
...
begin
  ...
  if (v_accuracy is not null and v_accuracy not between 0 and 100000)
    or (v_confidence is not null and v_confidence not between 0 and 1) then
    raise exception 'invalid_address_quality' using errcode='22023';end if;
  ...
  if v_kind='manual_map' then
    if v_accuracy is not null or v_confidence is not null
      or (v_previous_lat is null) is distinct from (v_previous_lng is null) then
      raise exception 'invalid_manual_address_evidence' using errcode='22023';
    end if;
    if v_previous_lat is not null and not (
      exists(select 1 from jsonb_array_elements(v_item.candidates) as candidate(value)
        where jsonb_typeof(candidate.value->'latitude')='number'
          and jsonb_typeof(candidate.value->'longitude')='number'
          and (candidate.value->>'latitude')::double precision=v_previous_lat
          and (candidate.value->>'longitude')::double precision=v_previous_lng)
      or exists(select 1 from public.canonical_addresses canonical
        where canonical.tenant_id=v_tenant and canonical.id=v_item.canonical_address_id
          and canonical.latitude=v_previous_lat and canonical.longitude=v_previous_lng)
    ) then
      raise exception 'manual_address_previous_point_mismatch' using errcode='22023';
    end if;
  end if;
  v_details:=jsonb_build_object('selected_label',left(coalesce(v_label,'Ponto ajustado no mapa'),500),
    'selection',v_kind,'previous_lat',v_previous_lat,
    'previous_lng',v_previous_lng,'request_id',v_request);
