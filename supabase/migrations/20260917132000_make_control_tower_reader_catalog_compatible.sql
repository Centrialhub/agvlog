-- Some production catalogs predate the private Control Tower helpers. Keep the
-- public read contract self-contained so an active trip never makes the page
-- unavailable and legacy telemetry remains subject to the same freshness rule.
...
begin
  ...
  from (
    select
      ...
      case when freshness.fresh then position.lat end as lat,
      case when freshness.fresh then position.lng end as lng,
      case when freshness.fresh then position.speed end as speed_kmh,
      case when freshness.fresh then position.heading end as heading,
      case
        when dispatch_trip.status not in ('in_transit', 'in_progress') then 'planned'
        when not _tracking then 'tracking_disabled'
        when not freshness.fresh then 'no_signal'
      ...
      case
        ...
        when not freshness.fresh then 'danger'
