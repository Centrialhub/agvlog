...
begin
  ...
  select jsonb_build_object(
    ...
    'stagnant_count',count(*) filter(where balance.quantity>0 and balance.first_inbound_at<statement_timestamp()-interval '30 days')::integer,
  ...
  ) into v_result from public.inventory_balances balance where balance.tenant_id=_tenant_id;
  return v_result;
end;
$function$;
...
comment on function public.get_inventory_summary_v1(uuid) is
  'Returns tenant inventory KPIs to any authenticated tenant member authorized to read the underlying inventory.';
