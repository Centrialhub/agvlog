create function finance_private.list_expenses(_tenant uuid,_filters jsonb default '{}') returns jsonb
...
begin
 ...
 if jsonb_typeof(_filters) is distinct from 'object' or page not between 1 and 1000000 or size not between 1 and 100
   ...
   or exists(select 1 from jsonb_object_keys(_filters) k where k not in('page','page_size','from','to','search','category','context','trip_id','missing_receipt')) then
   raise exception 'finance_invalid_expense_filters' using errcode='22023';end if;
 with filtered as materialized (
   select e.*,b.context,b.trip_id,b.driver_id,b.description batch_description,c.name cost_center_name,
   ...
   select category,sum(amount_cents)::text amount_cents,count(*) item_count from filtered group by category
 ), rows as (
   ...
   'total',(select count(*) from filtered),'total_cents',(select coalesce(sum(amount_cents),0)::text from filtered),
   'allocated_cents',(select coalesce(sum(allocated_cents),0)::text from filtered),
   'complement_cents',(select coalesce(sum(amount_cents-allocated_cents),0)::text from filtered),
   'missing_receipt_count',(select count(*) from filtered where receipt_path is null),
...
revoke all on function finance_private.list_expenses(uuid,jsonb) from public,anon,authenticated,service_role;
grant execute on function finance_private.list_expenses(uuid,jsonb) to authenticated;
create function public.list_finance_expenses(_tenant_id uuid,_filters jsonb default '{}') returns jsonb
language sql security invoker set search_path='' as $$select finance_private.list_expenses(_tenant_id,_filters);$$;
revoke all on function public.list_finance_expenses(uuid,jsonb) from public,anon,authenticated,service_role;
grant execute on function public.list_finance_expenses(uuid,jsonb) to authenticated;
