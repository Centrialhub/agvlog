...
begin
  ...
  if v_matched <> v_requested then
    raise exception 'Uma ou mais notas não existem mais ou não estão acessíveis' using errcode = 'P0002';
  end if;
  ...
  select
    ...
    jsonb_build_object(
      'audited', true,
      'cleared_legacy_processed_status', document.imported_note_status = 'processed'
    ),
    auth.uid(),
...
create or replace function public.protect_imported_note_status_v1()
returns trigger
language plpgsql
set search_path to 'public'
as $function$
begin
  if new.imported_note_status is distinct from old.imported_note_status
     and coalesce(current_setting('app.imported_note_audit_command', true), '') <> 'on' then
    raise exception 'Use audit_imported_notes_v1 para auditar notas importadas' using errcode = '42501';
  end if;
  return new;
end;
$function$;

drop trigger if exists protect_imported_note_status on public.fiscal_documents;
create trigger protect_imported_note_status
before update of imported_note_status on public.fiscal_documents
for each row execute function public.protect_imported_note_status_v1();
