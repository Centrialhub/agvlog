...
import { buildIngestionReportCsv, downloadReportPdf, type FieldCoverage, type ReviewItem, type IngestionReportRow } from '@/lib/ingestion/reportExports';
...
export default function IngestionReports() {
  ...
  const reportQuery = useQuery({
    queryKey: ['ingestion_report_index', currentTenant?.id, currentTenant?.timezone, from, to, batch, page, snapshotAt],
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.rpc.bind(supabase) as unknown as (
        ...
        _from: from ? dateOnlyUtcRange(from, currentTenant!.timezone).from : null,
        _to: to ? dateOnlyUtcRange(to, currentTenant!.timezone).toExclusive : null,
  ...
  const retryWithFreshSnapshot = () => {
    if (page === 1 && snapshotAt === null) void reportQuery.refetch();
    else resetPage();
  };
    ...
    <div className="animate-fade-in space-y-5 max-w-6xl">
      ...
      <Card>
        <CardContent className="p-0">
          {isError ? <div role="alert" className="p-6 text-sm">Não foi possível carregar os relatórios. <Button variant="link" onClick={retryWithFreshSnapshot}>Tentar novamente</Button></div> : isLoading ? (
          ...
          {!isError && !isLoading && (report?.total ?? 0) > 0 && (
            <div className="flex items-center justify-between border-t p-3">
              <Button variant="outline" disabled={page === 1} onClick={previousPage}>Anterior</Button>
...
export { buildIngestionReportCsv } from '@/lib/ingestion/reportExports';
