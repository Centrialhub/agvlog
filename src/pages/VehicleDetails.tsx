    ...
    <div className="animate-fade-in space-y-4">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="trips" className="space-y-4">
          {(() => {
            // Deduplicate trips: merge overlapping or near-identical entries
            const deduped = useMemo(() => {
              ...
              for (let i = 1; i < sorted.length; i++) {
                ...
                // Merge if gap < 5min or identical start times
                if (gap < 5) {
                  current = {
                    ...current,
                    id: current.id,
                    ...
                    distance_km_estimated: Math.max(current.distance_km_estimated || 0, next.distance_km_estimated || 0),
                    moving_time_seconds: Math.max(current.moving_time_seconds || 0, next.moving_time_seconds || 0),
                    ...
                    _merged_count: (current._merged_count || 1) + 1,
                  };
                } else {
                  merged.push(current);
            ...
            const totalKm = deduped.reduce((s: number, t: any) => s + (t.distance_km_estimated || 0), 0);
            const totalMoving = deduped.reduce((s: number, t: any) => s + (t.moving_time_seconds || 0), 0);
            const totalStopped = deduped.reduce((s: number, t: any) => s + (t.stopped_time_seconds || 0), 0);
            const maxSpeed = Math.max(0, ...deduped.map((t: any) => t.max_speed_kmh || 0));

            return (
              <>
                <div className="flex items-center gap-4">
                  <Input type="date" value={historyDate} onChange={e => setHistoryDate(e.target.value)} className="w-48" />
                  {trips.length !== deduped.length && (
                    <span className="text-xs text-muted-foreground">
                      {trips.length} registros brutos → {deduped.length} viagens consolidadas
                    </span>
                  )}
                </div>
                ...
                {deduped.length > 0 && (
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    <MiniKPI icon={<Route className="h-4 w-4" />} label="Viagens" value={String(deduped.length)} />
                    <MiniKPI icon={<Navigation className="h-4 w-4" />} label="Distância Total" value={`${totalKm.toFixed(1)} km`} />
                    <MiniKPI icon={<Activity className="h-4 w-4" />} label="Tempo em Mov." value={fmtHours(totalMoving)} />
                    <MiniKPI icon={<StopCircle className="h-4 w-4" />} label="Tempo Parado" value={fmtHours(totalStopped)} />
                    <MiniKPI icon={<Gauge className="h-4 w-4" />} label="Vel. Máxima" value={maxSpeed > 0 ? `${Math.round(maxSpeed)} km/h` : '—'} />
                  </div>
                )}
