...
const initial:FinanceAuditFilters={page:1,page_size:30,from:'',to:'',action:'',actor_id:'',actor_search:'',search:'',manual_only:true,snapshot_at:''};
...
function AuditWorkspace({tenant,actor}:{tenant:string;actor:string}){
  ...
  return <div className="space-y-5"><div><h1 className="text-2xl font-semibold">Auditoria financeira</h1>
    ...
    <form className="finance-filters" onSubmit={e=>{e.preventDefault();if(invalidDateRange)return;setFilters({...draft,page:1,snapshot_at:''});}}>
      ...
      <Button type="submit" disabled={invalidDateRange}>Filtrar</Button>
    </form>
    {invalidDateRange&&<p role="alert">A data inicial não pode ser posterior à data final.</p>}
    {filters.actor_id&&<p className="text-sm">Pessoa selecionada: {actorName} <Button variant="ghost" onClick={()=>{setDraft({...draft,actor_id:''});setFilters({...filters,page:1,actor_id:'',snapshot_at:''});setActorName('');}}>Remover pessoa selecionada</Button></p>}
    <p className="text-xs text-muted-foreground">Datas e horários no fuso configurado da empresa{data?.timezone?` (${data.timezone})`:''}. O nome mostrado é o registrado no momento da ação; alterações cadastrais posteriores não apagam essa identificação.</p>
    ...
    {data&&!query.error&&<><p>{data.total} evento(s) no filtro · {data.manual_count} intervenção(ões) manual(is)</p>
      <div className="space-y-3">{data.rows.map(row=><article key={row.id} className={`rounded border p-4 ${row.manual_intervention?'border-amber-600':''}`}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{financeAuditActions[row.action]||'Ação financeira registrada'}</p>
          <p>{row.actor_name} · {new Date(row.created_at).toLocaleString('pt-BR',{timeZone:data.timezone})}</p></div>
