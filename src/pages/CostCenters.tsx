...
import {
  ...
  Filter, Search, BarChart3, PieChart as PieChartIcon,
  ChevronRight, Calendar, Tag, FileText
} from 'lucide-react';
import {
...
export default function CostCenters() {
  ...
  // Fetch all transactions linked to cost centers
  const { data: transactions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [payRes, recRes, bankRes, expRes, maintRes] = await Promise.all([
        ...
        supabase.from('receivables').select('id, amount, description, created_at, status').eq('tenant_id', currentTenant.id), // Receivables usually don't have CC yet, but we'll check
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Gestão e KPIs de distribuição financeira por centro de custo
          </p>
        </div>
