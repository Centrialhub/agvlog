      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        <Card className="p-4">
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
              {SEFAZ_STATUSES.map((s) => (<label key={s} className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox checked={(draft.statuses ?? []).includes(s)} onCheckedChange={() => toggleStatus(s)}/>
          ...
          <div className="mt-4 flex items-center gap-4 flex-wrap">
            ...
            {['all', 'yes', 'no'].map((v) => (<label key={v} className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="radio" name="cce" checked={(draft.correctionLetter ?? 'all') === v} onChange={() => setDraft({ ...draft, correctionLetter: v })}/>
        ...
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between gap-3 flex-wrap border-b bg-muted/30 px-3 py-2">
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline" disabled={downloadableRows.length === 0 || bulkBusy} onClick={() => setChecked(new Set(downloadableRows.map((r) => r.id)))}>
                Selecionar todos os filtrados ({downloadableRows.length})
              </Button>
              <Button size="sm" disabled={checkedRows.length === 0 || bulkBusy} onClick={() => bulkDownload('pdf')}>
                <FileText className="h-4 w-4"/> Baixar PDF único
              </Button>
              <Button size="sm" variant="outline" disabled={checkedRows.length === 0 || bulkBusy} onClick={() => bulkDownload('xml')}>
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {!isLoading && rows.length === 0 && (<TableRow><TableCell colSpan={11} className="text-center text-muted-foreground py-8">Nenhum CT-e encontrado com os filtros atuais.</TableCell></TableRow>)}
                {rows.map((r) => (<TableRow key={r.id} className="border-t hover:bg-muted/30 cursor-pointer" onClick={() => setSelected(r)}>
                    <TableCell className="px-3 py-2" onClick={(e) => e.stopPropagation()}>
                      <Checkbox checked={checked.has(r.id)} onCheckedChange={() => toggleRow(r.id)} disabled={!r.hub_document_id && !r.pdf_url && !r.xml_url} aria-label="Selecionar CT-e"/>
                    </TableCell>
                    ...
                    <TableCell className="px-3 py-2 text-xs max-w-xs truncate" title={r.sefaz_status_reason ?? ''}>
                      {r.sefaz_status_reason ? (<span className="inline-flex items-center gap-1 text-destructive">
                    ...
                    <TableCell className="px-3 py-2 text-right" onClick={(e) => e.stopPropagation()}>
                      <div className="inline-flex gap-1">
                        ...
                        {r.sefaz_status.endsWith('_error') && (<Button size="sm" variant="ghost" title="Reenviar à SEFAZ" onClick={() => resend.mutate(r.id, {
...
function CteDetail({ row, onClose }) {
    ...
    return (<>
      ...
      <div className="flex items-center gap-3 mb-4">
        {(row.sefaz_status === 'processed' || row.sefaz_status === 'processed_error') && (<Button size="sm" variant="outline" onClick={handleCancel} disabled={cancelCte.isPending}>
            <Ban className="h-4 w-4 mr-2"/> Cancelar CT-e
          </Button>)}
        {row.sefaz_status === 'cancelled' && (<Badge variant="outline" className="text-destructive border-destructive/30">Documento Cancelado</Badge>)}
        {row.sefaz_status === 'processing' && (<Badge variant="outline" className="text-amber-600 border-amber-500/30">Em processamento no Hub...</Badge>)}
      </div>
      ...
      {row.sefaz_status_reason && (<div className="rounded border border-destructive/30 bg-destructive/10 p-3 text-sm">
      ...
      <div>
        ...
        <div className="border rounded max-h-64 overflow-y-auto divide-y">
          {events.length === 0 && (<p className="text-xs text-muted-foreground p-3">Nenhum evento registrado ainda. A integração fiscal envia os eventos via webhook.</p>)}
          {events.map((e) => (<div key={e.id} className="p-2 text-xs">
      ...
      <div className="flex justify-end items-center gap-2">
        {row.sefaz_status === 'processed' && row.source === 'hub' && (<Button variant="destructive" size="sm" onClick={handleCancel} disabled={cancelCte.isPending}>
