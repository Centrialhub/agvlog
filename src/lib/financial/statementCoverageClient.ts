...
import {statementCoverageSchema,statementCoverageHistorySchema,coveragePendingSchema,coverageApprovalResultSchema,coverageReversalResultSchema,type CoveragePending} from './statementCoverageContract';
...
async function rpc(name:string,args:Record<string,unknown>){const {data,error}=await (supabase.rpc.bind(supabase) as unknown as Rpc)(name,args);if(error){if(['22023','23514','23505','40001','42501','55000'].includes(error.code||''))throw new CoverageRejectedError(error.message);throw new Error(error.message);}return data;}
...
export async function readStatementCoverageHistory(tenant:string,account:string,from:string,to:string,page:number){const data=statementCoverageHistorySchema.parse(await rpc('get_finance_statement_coverage_history',{_tenant_id:tenant,_account_id:account,_from:from,_to:to,_page:page}));if(data.tenant_id!==tenant||data.account_id!==account||data.from!==from||data.to!==to||data.page!==page)throw new Error('Histórico fora da cobertura.');return data;}
