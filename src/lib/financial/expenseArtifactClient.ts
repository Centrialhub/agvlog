...
export async function readExpenseArtifacts(tenant:string,expense:string){
 const result=await(supabase.rpc.bind(supabase) as unknown as Rpc)('get_finance_expense_receipt_artifacts',{_tenant_id:tenant,_expense_id:expense});if(result.error)throw result.error;
...
export async function sendExpenseArtifact(command:ExpenseArtifactCommand){
 const result=await(supabase.rpc.bind(supabase) as unknown as Rpc)('attach_finance_expense_receipt_artifact',{_payload:command});
