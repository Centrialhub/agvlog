...
export async function readSettlementExpenseContext(tenant:string,settlement:string,page:number){
 const {data,error}=await (supabase.rpc.bind(supabase) as unknown as Rpc)('get_finance_settlement_expense_context',{_tenant_id:tenant,_settlement_id:settlement,_page:page});
