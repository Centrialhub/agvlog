...
export async function readRecordedCostOperationalCoverage(tenant:string,from:string,to:string){
 const {data,error}=await(supabase.rpc.bind(supabase) as unknown as Rpc)('get_finance_recorded_cost_operational_coverage',{_tenant_id:tenant,_from:from||null,_to:to||null});
