...
export const settlementHistoryItemSchema=settlementLinkSchema.extend({reversal:z.object({id:uuid,actor_id:uuid,actor_name:z.string(),reason:z.string(),created_at:z.string()}).nullable()});
export const settlementMovementOptionsSchema=z.object({version:z.literal(1),tenant_id:uuid,payment_id:uuid,settlement_id:uuid,amount_cents:cents,page:z.number().int().positive(),page_size:z.literal(20),total:z.number().int().nonnegative(),link:settlementLinkSchema.nullable(),history:z.array(settlementHistoryItemSchema).max(20),history_page:z.literal(1),history_page_size:z.literal(20),history_has_more:z.boolean(),rows:z.array(z.object({id:uuid,description:z.string(),occurred_on:z.string(),amount_cents:cents,remaining_cents:cents,beneficiary_name:z.string(),account_name:z.string()})).max(20)});
export const settlementMovementHistorySchema=z.object({version:z.literal(1),tenant_id:uuid,payment_id:uuid,settlement_id:uuid,page:z.number().int().positive(),page_size:z.literal(20),has_more:z.boolean(),rows:z.array(settlementHistoryItemSchema).max(20)});
