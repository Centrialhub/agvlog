...
export const statementCursorSchema=z.object({created_at:z.string(),id:uuid}).strict();
export type StatementCursor=z.infer<typeof statementCursorSchema>;
export interface StatementCursorFilters extends Omit<StatementListFilters,'page'>{cursor:StatementCursor|null}
...
export const statementCursorListSchema=z.object({version:z.literal(2),tenant_id:uuid,page_size:z.number().int().positive(),cursor:statementCursorSchema.nullable(),next_cursor:statementCursorSchema.nullable(),has_more:z.boolean(),total:z.number().int().nonnegative(),rows:z.array(statementSummarySchema)});
