...
const creditFields={discount_cents:cents.optional(),loss_cents:cents.optional(),adjustment_cents:cents.optional(),cash_received_cents:cents.optional(),credit_applied_cents:cents.optional(),settled_cents:cents.optional()};
export const receivablePortfolioSchema=z.object({version:z.literal(1),tenant_id:uuid,from:date.nullable(),to:date.nullable(),client_id:uuid.nullable(),as_of:date,total_titles:count,canceled_titles:count,invalid_titles:count,totals_valid:z.boolean(),...creditFields,nominal_cents:cents,received_allocated_cents:cents,open_cents:cents,overdue_cents:cents,status_rows:z.array(z.object({status:z.string(),count,...creditFields,nominal_cents:cents,received_allocated_cents:cents,open_cents:cents}))}).superRefine((data,ctx)=>{
 const creditRows=[data,...data.status_rows];
 const hasAdjustment=creditRows.some(r=>r.adjustment_cents!==undefined||r.discount_cents!==undefined||r.loss_cents!==undefined);
 if(hasAdjustment)for(const r of creditRows){const a=[r.discount_cents,r.loss_cents,r.adjustment_cents];if(a.some(v=>v===undefined)||a.some(v=>data.totals_valid?v===null:v!==null)||a.every(v=>typeof v==='string'&&/^\d+$/.test(v))&&BigInt(r.discount_cents!)+BigInt(r.loss_cents!)!==BigInt(r.adjustment_cents!))ctx.addIssue({code:'custom',message:'Descontos e perdas inconsistentes.'});}
 const hasCredit=creditRows.some(r=>r.cash_received_cents!==undefined||r.credit_applied_cents!==undefined||r.settled_cents!==undefined);
 if(hasAdjustment&&!hasCredit)ctx.addIssue({code:'custom',message:'Composição incompleta.'});
 if(hasCredit){for(const r of creditRows){const fields=[r.cash_received_cents,r.credit_applied_cents,r.settled_cents];if(fields.some(v=>v===undefined)||fields.some(v=>data.totals_valid?v===null:v!==null))ctx.addIssue({code:'custom',message:'Composição de crédito incompleta.'});if(fields.every(v=>typeof v==='string'&&/^\d+$/.test(v))&&typeof r.received_allocated_cents==='string'&&/^\d+$/.test(r.received_allocated_cents)&&(BigInt(r.cash_received_cents!)+BigInt(r.credit_applied_cents!)+(typeof r.adjustment_cents==='string'&&/^\d+$/.test(r.adjustment_cents)?BigInt(r.adjustment_cents):0n)!==BigInt(r.settled_cents!)||r.settled_cents!==r.received_allocated_cents))ctx.addIssue({code:'custom',message:'Liquidação e composição de crédito divergem.'});}
 for(const key of ['cash_received_cents','credit_applied_cents','settled_cents','discount_cents','loss_cents','adjustment_cents'] as const)if(typeof data[key]==='string'&&/^\d+$/.test(data[key]!)&&data.status_rows.every(r=>typeof r[key]==='string'&&/^\d+$/.test(r[key]!))&&data.status_rows.reduce((sum,r)=>sum+BigInt(r[key]!),0n)!==BigInt(data[key]!))ctx.addIssue({code:'custom',message:'Totais de crédito divergem das categorias.'});}
 ...
 if(data.totals_valid&&values.every(value=>typeof value==='string'&&/^\d+$/.test(value))){
