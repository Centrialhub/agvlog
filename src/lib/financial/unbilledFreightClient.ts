...
async function rpc(name:string,args:Record<string,unknown>){const {data,error}=await (supabase.rpc.bind(supabase) as unknown as Rpc)(name,args);if(error)throw error;return data;}
