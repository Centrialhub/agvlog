...
export const legacyReceivableReverseCommandSchema=z.object({...base,payment_id:uuid,link_id:uuid,reason}).strict();
...
export const legacyReceivableContextSchema=z.object({version:z.literal(1),tenant_id:uuid,payment_id:uuid,revision:z.string().min(1),page_revision:z.string().regex(/^[a-f0-9]{32}$/),page:z.number().int().positive(),page_size:z.literal(20),total:z.number().int().nonnegative(),
