...
const contextSchema=z.object({version:z.literal(1),tenant_id:id,actor_id:id,receivable_id:id,invoice_id:id.nullable(),report_id:id.nullable(),reference:z.string(),revision,
 status:z.string(),amount_cents:cents.positive(),received_cents:cents,open_cents:cents,...receivableCreditNumberFields,balance_adjustment_revision:revision.optional(),balance_adjustment_event_count:z.number().int().nonnegative().optional(),credit_revision:revision.optional(),credit_application_count:z.number().int().nonnegative().optional(),requires_reconciliation:z.boolean(),reconciliation_reason:z.string().nullable(),
...
export function parseFinancialContext(value:unknown,tenant:string,actor:string,receivable:string){
 ...
 if((row.balance_adjustment_revision===undefined)!==(row.balance_adjustment_event_count===undefined)||(row.credit_revision===undefined)!==(row.credit_application_count===undefined)||!creditCompositionValid(row,row.received_cents)||(row.settled_cents===null&&row.can_receive)||row.tenant_id!==tenant||row.actor_id!==actor||row.receivable_id!==receivable||(row.source_issue&&(row.can_receive||!row.source_revision))||(!row.requires_reconciliation&&(row.status==='cancelled'?row.received_cents!==0||row.open_cents!==0:row.received_cents+row.open_cents!==row.amount_cents)))
