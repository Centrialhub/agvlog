...
export const payableMovementOptionsSchema=z.object({version:z.literal(1),tenant_id:uuid,payable_id:uuid,page_revision:z.string().regex(/^[a-f0-9]{32}$/),page:z.number().int().positive(),total:z.number().int().nonnegative(),
...
export const payablePaymentHistorySchema=z.object({version:z.literal(1),tenant_id:uuid,payable_id:uuid,page_revision:z.string().regex(/^[a-f0-9]{32}$/),page:z.number().int().positive(),total:z.number().int().nonnegative(),rows:z.array(z.object({
 id:uuid,tenant_id:uuid,payable_id:uuid,amount:z.number(),paid_at:z.string(),method:z.string(),notes:z.string().nullable(),attachment_url:z.string().nullable(),account_name:z.string().nullable(),link_id:uuid.nullable(),movement_id:uuid.nullable(),link_origin:z.enum(['canonical','legacy_adoption']).nullable(),
