...
export async function readReceivablesPage(tenant:string,filters:ReceivableListFilters,page:number){
 const {data,error}=await (supabase.rpc.bind(supabase) as unknown as Rpc)('get_finance_receivables_page_by_origin',{_tenant_id:tenant,_search:filters.search,_status:filters.status,_client_id:filters.client==='all'?null:filters.client,_from:filters.from||null,_to:filters.to||null,_page:page,_origin:filters.origin||'all'});
