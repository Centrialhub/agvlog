...
export function statementImportErrorMessage(error:unknown):string{
  ...
  const translations:Record<string,string>={invalid_date:'Data inválida para o formato selecionado.',invalid_excel_date:'Data numérica Excel inválida.',
    ...
    no_transactions:'Nenhum lançamento foi encontrado após o cabeçalho.',formula_cells_require_review:'O arquivo contém fórmulas em uma ou mais abas. Use o extrato original com os valores fornecidos pelo banco.',
    workbook_macros_require_review:'O arquivo contém macro ou rotina automática. Exporte um extrato sem macros diretamente do banco.',
    workbook_resource_limit:'A planilha excede o limite seguro de abas ou nomes de abas.',worksheet_too_large:'A planilha excede o limite seguro de linhas, colunas ou células.',
    workbook_cell_invalid:'A planilha contém célula de erro ou valor numérico inválido.',workbook_cell_too_large:'A planilha contém uma célula de texto acima do limite seguro.',
