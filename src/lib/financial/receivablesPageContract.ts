...
type ConsistentPage=Pick<z.infer<typeof pageObject>,'tenant_id'|'total'|'total_unfiltered'|'rows'>&{origin_filter?:string};
const consistency=(data:ConsistentPage,ctx:z.RefinementCtx)=>{if(data.rows.some(row=>!creditCompositionValid(row,legacyReceivableCents(row.received_amount))))ctx.addIssue({code:'custom',message:'Composição da liquidação inválida.'});if(data.total>data.total_unfiltered||data.rows.some(row=>row.tenant_id!==data.tenant_id)||new Set(data.rows.map(row=>row.id)).size!==data.rows.length)ctx.addIssue({code:z.ZodIssueCode.custom,message:'Lista de títulos inconsistente.'});if(data.origin_filter!==undefined&&data.rows.some(row=>!['unloading','fiscal','other'].includes(String(row.origin_kind))||(data.origin_filter!=='all'&&row.origin_kind!==data.origin_filter)))ctx.addIssue({code:z.ZodIssueCode.custom,message:'Origem dos títulos inconsistente.'});};
...
export const receivablePageCursorSchema=z.object({created_at:z.string(),id:uuid}).strict();
export type ReceivablePageCursor=z.infer<typeof receivablePageCursorSchema>;
const cursorObject=originObject.omit({version:true,page:true}).extend({version:z.literal(3),cursor:receivablePageCursorSchema.nullable(),next_cursor:receivablePageCursorSchema.nullable(),has_more:z.boolean()});
export const receivablesCursorPageSchema=cursorObject.superRefine(consistency);
