...
export async function recordCashOpening(command:CashOpeningCommand){
 ...
 const {data,error}=await (supabase.rpc.bind(supabase) as unknown as (name:string,args:Record<string,unknown>)=>PromiseLike<{data:unknown;error:{message:string;code?:string}|null}>)('record_finance_cash_opening',{_payload:command});
