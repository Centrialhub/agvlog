...
export async function readPeriodMoneyPackage(tenant:string,from:string,to:string,accountIds:string[]){
 ...
 const {data,error}=await(supabase.rpc.bind(supabase) as unknown as Rpc)('get_finance_period_money_package',{_tenant_id:tenant,_from:from,_to:to,_account_ids:accountIds});
