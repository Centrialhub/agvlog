...
const openingHistory=opening.extend({reversal:z.object({id:uuid,actor_id:uuid,actor_name:z.string(),reason:z.string(),created_at:z.string()}).nullable()});
export const accountOpeningContextSchema=z.object({version:z.literal(1),tenant_id:uuid,account_id:uuid,from:date,to:date,opening:opening.extend({evidence_status:z.enum(['valid','requires_review'])}).nullable(),book:z.object({opening_cents:cents,in_cents:cents,out_cents:cents,closing_cents:cents}).nullable(),history:z.array(openingHistory).max(20),history_has_more:z.boolean(),can_close:z.literal(false)}).superRefine((data,ctx)=>{
...
export const accountOpeningHistorySchema=z.object({version:z.literal(1),tenant_id:uuid,account_id:uuid,page:z.number().int().positive(),page_size:z.literal(20),has_more:z.boolean(),rows:z.array(openingHistory).max(20)});
