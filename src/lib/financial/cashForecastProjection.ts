...
const origin = z.object({
  ...
  if (BigInt(row.fulfilled_cents) + BigInt(row.reserved_credit_cents) > BigInt(row.nominal_cents))
    ctx.addIssue({ code: 'custom', message: 'Recebimentos, pagamentos e cr̩ditos excedem a origem.' });
  if ((row.expected_on === null) !== (row.expected_date_source === 'unknown'))
    ctx.addIssue({ code: 'custom', message: 'A previṣo da data exige sua origem.' });
...
const validateBasis = (basis: z.infer<typeof forecastBasisObject> | z.infer<typeof companyBasisObject>, ctx: z.RefinementCtx) => {
  ...
  if (new Set(basis.account_ids).size !== basis.account_ids.length)
    ctx.addIssue({ code: 'custom', message: 'Conta repetida no escopo da previṣo.' });
  if (new Set(basis.origins.map(row => row.economic_key)).size !== basis.origins.length
    ...
    ctx.addIssue({ code: 'custom', message: 'A mesma origem econ̫mica aparece mais de uma vez.' });
