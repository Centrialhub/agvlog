...
export const uploadArtifactSchema=z.object({
 ...
 derivative:z.object({bucket:z.literal('upload-validated'),path:z.string(),sha256:hash,size_bytes:z.number().int().positive().max(20971520),mime:z.enum(['application/json','image/jpeg','image/png']),method:z.enum(['native-ofx-v1','strict-csv-matrix-v1','strict-workbook-matrix-v1','jpeg-png-reencode-v1']),financial_mapping_required:z.boolean()}).nullable(),
}).superRefine((d,c)=>{
 ...
 if(d.derivative){
  ...
  const valid=d.state==='validated_data'?
   d.derivative.mime==='application/json'&&((d.original.format==='ofx'&&d.derivative.method==='native-ofx-v1'&&!d.derivative.financial_mapping_required)||(d.original.format==='csv'&&d.derivative.method==='strict-csv-matrix-v1'&&d.derivative.financial_mapping_required)||(d.original.format==='xlsx'&&d.derivative.method==='strict-workbook-matrix-v1'&&d.derivative.financial_mapping_required)):
...
export function uploadArtifactStatus(a:UploadArtifact):string{
 ...
 return a.derivative?.financial_mapping_required?`${a.original.format==='xlsx'?'Planilha':'Dados CSV'} validada. Ainda é necessário mapear as colunas e conferir a conta e os valores.`:'Dados OFX validados. A importação e a conferência financeira ainda precisam ser concluídas.';
}
