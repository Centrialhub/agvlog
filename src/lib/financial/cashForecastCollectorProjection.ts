...
export function projectCollectedCashForecast(input:unknown,expected:Expected){
 ...
 const basis:CashForecastCompanyBasis={version:2,tenant_id:collection.tenant_id,account_ids:collection.account_scope.account_ids,captured_at:collection.captured_at,cutoff:collection.cutoff,period_end:collection.period_end,base:collection.base,
  origins:collection.origins.flatMap(row=>row.valid?[{economic_key:row.economic_key,source_table:row.source_table,source_id:row.source_id,source_revision:row.source_revision,direction:row.direction,scenario:row.scenario,nominal_cents:row.nominal_cents!,fulfilled_cents:row.fulfilled_cents!,reserved_credit_cents:row.reserved_credit_cents!,...(row.adjustment_cents===undefined?{}:{discount_cents:row.discount_cents!,loss_cents:row.loss_cents!,adjustment_cents:row.adjustment_cents!}),expected_on:row.expected_on,expected_date_source:row.expected_date_source}]:[]),
