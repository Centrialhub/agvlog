...
export async function readUnloadingProjectionRepairContext(request:z.infer<typeof scope>){
 ...
 const {data,error}=await(supabase.rpc.bind(supabase) as unknown as Rpc)('get_finance_unloading_projection_repair_context',{_tenant_id:input.tenantId,_charge_id:input.chargeId});
