...
import { onlyDigits, normalizeUf, normalizeCep, normalizeIbgeCity, normalizeCityName, normalizeCpfCnpj, normalizePhone, fiscalText, money, isValidEmail, } from './fiscalAddress';
...
export function buildNFSeEmitPayload({ doc, emitter, environment, callbackUrl, attempt = 0 }) {
    if (!emitter)
    ...
    // ---------------------------------------------------------------------------
    const endRaw = (emitter.endereco || {});
    const prestadorMissing = [];
    if (!normalizeCpfCnpj(emitter.cnpj))
        prestadorMissing.push('CNPJ do emitente');
    if (!fiscalText(emitter.razao_social, 150))
        prestadorMissing.push('razão social do emitente');
    if (!emitter.im)
        prestadorMissing.push('inscrição municipal do emitente');
    const prestadorCityCode = normalizeIbgeCity(emitter.city_code) ||
        normalizeIbgeCity(endRaw.city_code) ||
        normalizeIbgeCity(endRaw.codigo_ibge);
    if (!prestadorCityCode)
        prestadorMissing.push('código IBGE do município do emitente');
    const prestadorCep = normalizeCep(endRaw.cep);
    if (!prestadorCep)
        prestadorMissing.push('CEP do emitente (8 dígitos válidos)');
    const prestadorUf = normalizeUf(endRaw.uf || endRaw.estado);
    if (!prestadorUf)
        prestadorMissing.push('UF do emitente');
    if (!fiscalText(endRaw.logradouro || endRaw.endereco, 120))
        prestadorMissing.push('logradouro do emitente');
    if (!fiscalText(endRaw.bairro, 60))
        prestadorMissing.push('bairro do emitente');
    ...
    const env = environment || emitter?.metadata?.environment || 'production';
    const end = endRaw;
    ...
    const baseDiscriminacao = String(doc.description ||
        items.map((it) => `${it.description} (${num(it.quantity)}x R$ ${num(it.unit_value).toFixed(2)})`).join(' | ') ||
    ...
    const payload = nonEmpty({
        ...
        prestador: {
            ...
            telefone: normalizePhone(emitter.telefone || emitter.phone) || undefined,
            email: isValidEmail(emitter.email) ? String(emitter.email).trim() : undefined,
            endereco: {
        ...
        servico: {
            ...
            itens: items
                .filter((it) => fiscalText(it?.description, 120))
                .map((it) => ({
