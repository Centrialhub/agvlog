...
export function buildEmitterExclusiveUse(input) {
    ...
    if (ins?.name) {
        ...
        if (ins.cnpj)
            seg.push(`CNPJ ${digits(ins.cnpj)}`);
        if (ins.policy)
            seg.push(`APOLICE ${ins.policy}`);
        if (ins.endorsement)
            seg.push(`AVERBACAO/CGC ${ins.endorsement}`);
        if (input.insuredAmount)
            seg.push(`VALOR SEGURADO R$ ${Number(input.insuredAmount).toFixed(2)}`);
        if (input.insuranceValue)
            seg.push(`SEGURO R$ ${Number(input.insuranceValue).toFixed(2)}`);
        parts.push(seg.join(' - '));
    }
    ...
    if (driverName && driverName !== '.') {
        ...
        if (input.driver?.cpf)
            drv.push(`CPF ${digits(input.driver.cpf)}`);
        parts.push(drv.join(' - '));
    }
...
export function computeIcmsAmounts(params) {
    ...
    if (embutido) {
        ...
        const base = providedBase != null && providedBase > 0 ? providedBase : freight / (1 - aliq / 100);
 ...
 */
function buildIcmsBlock(icms, freightValue, taxRegime) {
    ...
    if (isento) {
        const motivo = icms.motivo ?? 12; // 12 = Outros
        block.motivo = motivo;
...
function buildComponentes(params) {
    ...
    if (insurance > 0)
        items.push({ nome: COMPONENT_LABELS.insurance_value, valor: round2(insurance), soma: true });
    for (const [k, v] of Object.entries(comp)) {
        if (k === 'freight_weight' || k === 'insurance_pct' || k === 'insurance_value')
            continue;
        const n = Number(v || 0);
        if (n > 0) {
            items.push({
                nome: COMPONENT_LABELS[k] || k.toUpperCase(),
                valor: round2(n),
                soma: true,
            });
        }
    ...
    if (icms > 0)
        items.push({ nome: 'ICMS', valor: round2(icms), soma: params.icmsEmbutido === true });
    return items;
}
export function buildCtePayload(input) {
    const missing = [];
    const warnings = [];
    if (!input.emitter)
        missing.push('Emitente');
    else {
        if (!digits(input.emitter.cnpj))
            missing.push('CNPJ do emitente');
    }
    if (!input.remitter)
        missing.push('Remetente');
    else if (!digits(input.remitter.cnpj) && !digits(input.remitter.cpf)) {
        missing.push('CNPJ/CPF do remetente');
    }
    if (!input.recipient)
        missing.push('Destinatário');
    ...
    if (!input.nature)
        missing.push('Natureza da operação');
    if (!input.invoices?.length)
        missing.push('NFs referenciadas');
    if (!input.totals || !(input.totals.freight_value > 0))
        missing.push('Valor do frete');
    ...
    const freightComposition = input.freightComposition
        ? Object.fromEntries(Object.entries(input.freightComposition).filter(([, v]) => v != null))
        : undefined;
    ...
    const fretePeso = Number((Number(input.freightComposition?.freight_weight || 0) > 0
        ? Number(input.freightComposition?.freight_weight)
        : input.totals.freight_value).toFixed(2));
    ...
    const insuredAmount = input.insurer?.insured_amount ?? (input.insurer ? input.totals.cargo_value || null : null);
    ...
    const componentes = buildComponentes({
        ...
        insuranceValue,
        icmsValor: icmsBlock?.vICMS ?? null,
    ...
    const totalServico = Number(componentes
    ...
    const inicio = buildLocation(input.origin) ||
    ...
    if (!emitterRegimeRaw) {
        warnings.push('Regime tributário do emitente não cadastrado — assumindo regime normal (CRT 3). Configure em Configurações → Emitentes para garantir a impressão do ICMS no DACTE.');
    }
    const payload = {
        ...
        payload: {
            ...
            compl: usoExclusivoEmitente
                ...
                : undefined,
            emitente: serializeParty(input.emitter
                ? {
            ...
            mercadoria: input.cargo
                ? (() => {
                    const merc = {};
                    if (input.cargo?.content)
                        merc.content = input.cargo.content;
                    const produto = input.cargo?.predominant_product || input.cargo?.content;
                    if (produto)
                        merc.produto = produto;
                    if (input.cargo?.species)
                        merc.species = input.cargo.species;
