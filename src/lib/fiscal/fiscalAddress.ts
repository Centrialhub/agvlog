...
export function normalizeUf(v?: string | null): string | null {
  const raw = String(v ?? '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .toUpperCase().trim();
  if (!raw) return null;
  if (raw.length === 2 && VALID_UF.has(raw)) return raw;
 ...
 */
export function normalizeCep(v?: string | null): string | null {
  const d = onlyDigits(v);
  if (d.length < 7 || d.length > 8) return null;
  const cep = d.padStart(8, '0');
  if (/^0{8}$/.test(cep)) return null;
  return cep;
}

/** Código IBGE do município: exatamente 7 dígitos, senão null. */
export function normalizeIbgeCity(v?: string | null): string | null {
  const d = onlyDigits(v);
  return d.length === 7 ? d : null;
}

/** Nome de município: sem código numérico, sem UF colada, comprimento limitado. */
export function normalizeCityName(v?: string | null): string | null {
  const raw = String(v ?? '').trim();
  if (!raw) return null;
  if (/^\d+$/.test(raw)) return null; // era um código, não um nome
  return raw.replace(/\s*[-/]\s*[A-Z]{2}$/i, '').slice(0, 60);
}

/** CPF (11) ou CNPJ (14). Retorna null para qualquer outro tamanho. */
export function normalizeCpfCnpj(v?: string | null): string | null {
  const d = onlyDigits(v);
...
export function fiscalText(v: unknown, max: number): string | null {
