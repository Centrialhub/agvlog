...
import { buildCtePayload, computeIcmsAmounts } from '@/lib/fiscal/cteBuilder';
...
import { applyInsuranceProfileToBatch, hasInsuranceProfile, preserveInsurerFields, } from '@/lib/fiscal/insuranceProfile';
import { buildClientIndex, fillPartyFieldsFromRegistry, resolveParty, } from '@/lib/fiscal/partyRegistry';
/** Recalcula base/valor do ICMS respeitando o regime embutido (por dentro). */
function recalcIcms(freight, aliq, embutido, isento, providedBase) {
...
function groupToEditable(g, defaultEmitterId) {
...
function toBuildInput(e, emitter, environment = 'sandbox', clients = []) {
    // Completa lacunas das partes com o cadastro local (CNPJ, IE, endereço).
    const registry = buildClientIndex(clients);
    const enrichParty = (name, cnpj, fallbackAddress, ieOverride, clientId) => resolveParty(registry, { id: clientId, name, cnpj, ie: ieOverride }, fallbackAddress);
    return {
        emitter: emitter
            ? {
                id: emitter.id,
        ...
        recipient: enrichParty(e.recipientName, e.recipientCnpj, { city: e.recipientCity, state: e.recipientState }, e.recipientIe, e.clientId),
        ...
        takerParty: e.takerRole === 'terceiro'
...
export function CteEmissionPreviewDialog({ open, onOpenChange, groups }) {
    ...
    const defaultEmitter = emitters.find((e) => e.is_default && e.active) || emitters[0];
    ...
    const groupsSignature = useMemo(() => groups.map((g) => `${g.key}:${(g.fiscal_document_ids || []).join('.')}`).join('|'), [groups]);
    useEffect(() => {
        if (!currentTenant?.id)
            return;
        (async () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            const patched = await Promise.all(baseItems.map(async (it) => {
                if (it.loadIds.length === 0)
                    return it;
    ...
    useEffect(() => {
        if (!open || !hasInsuranceProfile(insuranceProfile))
            return;
        setItems((prev) => {
            if (prev.length === 0)
                return prev;
    ...
    useEffect(() => {
        if (!open || items.length === 0 || clients.length === 0)
            return;
        const registry = buildClientIndex(clients);
        let changed = false;
        ...
        items.some((it) => !it.remitterCnpj || !it.remitterIe || !it.recipientCnpj || !it.recipientIe ||
            !it.recipientCity || !it.recipientState),
    ]);
    const active = items[activeIdx];
    const emitterForActive = useMemo(() => emitters.find((e) => e.id === active?.emitterId) || defaultEmitter, [emitters, active?.emitterId, defaultEmitter]);
    ...
    useEffect(() => {
        ...
        const regime = emitterForActive?.regime_tributario;
        ...
        if (Math.abs(active.icmsAliquota - suggested) < 0.001)
            return;
    ...
    const activeCteCred = useMemo(() => activeCreds.find((c) => c.doc_scope === 'cte' && c.enabled) ||
        activeCreds.find((c) => c.doc_scope === 'all' && c.enabled) ||
        null, [activeCreds]);
    const activeEnvironment = activeCteCred?.environment === 'production' ? 'production' : 'sandbox';
    ...
    const allValid = items.every((it) => {
        const em = emitters.find((e) => e.id === it.emitterId) || defaultEmitter;
        return buildCtePayload(toBuildInput(it, em, 'sandbox', clients)).ok;
    });
    const insuranceErrors = useMemo(() => active
        ? validateInsurance({
    ...
    async function transmit(forcedItems) {
        ...
        try {
            for (let i = 0; i < itemsToTransmit.length; i++) {
                ...
                const { data: itCreds } = await supabase
                    ...
                    .eq('emitter_id', em?.id)
                    .eq('enabled', true);
                const itCred = (itCreds || []).find((c) => c.doc_scope === 'cte') ||
                    (itCreds || []).find((c) => c.doc_scope === 'all') ||
                    null;
                const itEnv = itCred?.environment === 'production' ? 'production' : 'sandbox';
                try {
                    ...
                    setItems((arr) => arr.map((x) => (x.key === it.key ? { ...x, transmitted: 'ok' } : x)));
                    okCount++;
                }
                catch (err) {
                    setItems((arr) => arr.map((x) => x.key === it.key ? { ...x, transmitted: 'error', transmitMessage: err?.message } : x));
    ...
    function handleTransmitClick() {
        ...
        if (sameCityItems.length > 0) {
            const sameCityNames = sameCityItems
                ...
                .join(", ");
            showAlert("Atenção: Destino igual ao Emitente", `As seguintes notas têm destino para a mesma cidade do emitente: ${sameCityNames}. Deseja prosseguir?`, "warning", {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          {!activeCteCred && emitterForActive && (<Badge variant="destructive">
              Sem credencial CT-e — usará token padrão (risco)
            </Badge>)}
          {activeCteCred && (<span className="text-muted-foreground">
              scope: {activeCteCred.doc_scope} · env: {activeCteCred.environment}
            </span>)}
          <label className="ml-auto flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={bulkEdit} onChange={(e) => setBulkEdit(e.target.checked)} className="h-3 w-3"/>
        ...
        <div className="grid grid-cols-[220px_1fr] gap-4">
          ...
          <ScrollArea className="h-[540px] rounded-md border p-2">
            ...
            return (<button key={it.key} onClick={() => setActiveIdx(i)} className={`w-full text-left rounded p-2 text-xs transition-colors ${i === activeIdx ? 'bg-primary/10 border border-primary' : 'hover:bg-muted'}`}>
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-mono">#{i + 1}</span>
                      {it.transmitted === 'ok' ? (<Badge variant="default" className="text-[10px]">Enviado</Badge>) : it.transmitted === 'error' ? (<Badge variant="destructive" className="text-[10px]">Erro</Badge>) : ok ? (<CheckCircle2 className="h-3 w-3 text-green-600"/>) : (<AlertCircle className="h-3 w-3 text-yellow-600"/>)}
                    </div>
                    ...
                    {it.invoices.length > 0 && (<div className="mt-1 truncate font-mono text-[10px] text-muted-foreground" title={it.invoices.map((n) => n.number || '—').join(', ')}>
                        NF: {it.invoices.map((n) => n.number || '—').join(', ')}
                      </div>)}
                  </button>);
        })}
            </div>
          ...
          <div className="max-h-[540px] overflow-y-auto pr-1">
            {!validation.ok && (<Alert variant="destructive" className="mb-3">
                <AlertCircle className="h-4 w-4"/>
                <AlertDescription>
                  <strong>Campos obrigatórios ausentes:</strong> {validation.missing.join(', ')}
                </AlertDescription>
              </Alert>)}
            {validation.warnings.length > 0 && (<Alert className="mb-3">
                <AlertCircle className="h-4 w-4"/>
                <AlertDescription>{validation.warnings.join(' • ')}</AlertDescription>
              </Alert>)}
            {active.transmitted === 'error' && (<Alert variant="destructive" className="mb-3">
              ...
              <TabsContent value="partes" className="space-y-3 pt-3">
                <div>
                  ...
                  <Select value={active.emitterId} onValueChange={(v) => patch({ emitterId: v })}>
                    ...
                    <SelectContent>
                      {emitters.map((e) => (<SelectItem key={e.id} value={e.id}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <Input value={active.remitterIe} onChange={(e) => patch({ remitterIe: e.target.value })} placeholder="ISENTO se não contribuinte"/>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                  ...
                  <div>
                    ...
                    <Input value={active.recipientIe} onChange={(e) => patch({ recipientIe: e.target.value })} placeholder="ISENTO se não contribuinte"/>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <Select value={active.consigneeClientId || 'none'} onValueChange={(v) => {
            if (v === 'none')
                return patch({ consigneeClientId: null, consigneeName: '', consigneeCnpj: '' });
            ...
            patch({
                ...
                <div className="grid grid-cols-3 gap-2 pt-2 border-t">
                  <div>
                    ...
                    {insuranceErrors.name && (<p className="text-[11px] text-destructive">{insuranceErrors.name}</p>)}
                  </div>
                  <div>
                    <Label>CNPJ seguradora</Label>
                    <Input value={formatCnpj(active.insurerCnpj)} inputMode="numeric" placeholder="00.000.000/0000-00" onChange={(e) => patch({ insurerCnpj: onlyDigits(e.target.value).slice(0, 14) })}/>
                    {insuranceErrors.cnpj && (<p className="text-[11px] text-destructive">{insuranceErrors.cnpj}</p>)}
                  </div>
                  <div>
                    <Label>Apólice</Label>
                    <Input value={active.insurerPolicy} onChange={(e) => patch({ insurerPolicy: e.target.value })}/>
                    {insuranceErrors.policy && (<p className="text-[11px] text-destructive">{insuranceErrors.policy}</p>)}
                  </div>
                  <div>
                    <Label>Nº averbação / CGC</Label>
                    <Input value={active.insurerEndorsement} onChange={(e) => patch({ insurerEndorsement: e.target.value })}/>
                    {insuranceErrors.endorsement && (<p className="text-[11px] text-destructive">{insuranceErrors.endorsement}</p>)}
                  </div>
                  <div>
                    <Label>Valor segurado (R$)</Label>
                    <Input type="number" step="0.01" value={active.insurerInsuredAmount || active.cargoValue || 0} onChange={(e) => patch({ insurerInsuredAmount: Number(e.target.value) })}/>
                  </div>
                  <div>
                    <Label>Seguro cobrado (R$)</Label>
                    <Input type="number" step="0.01" value={active.fcInsurance} onChange={(e) => patch({ fcInsurance: Number(e.target.value) })}/>
                  </div>
                </div>
                <div className="flex items-center gap-2 pt-1">
                  <Button type="button" variant="outline" size="sm" disabled={saveInsuranceProfile.isPending || !active.insurerName} onClick={async () => {
            try {
                ...
                toast.error('Falha ao salvar seguradora', { description: e?.message });
                  ...
                  {hasInsuranceProfile(insuranceProfile) && (<Button type="button" variant="ghost" size="sm" onClick={() => {
                ...
                {Object.keys(insuranceErrors).length > 0 && (<p className="text-[11px] text-amber-600">
              ...
              <TabsContent value="tomador" className="space-y-3 pt-3">
                ...
                <RadioGroup value={active.takerRole} onValueChange={(v) => patch({ takerRole: v })}>
                  {['remetente', 'destinatario', 'expedidor', 'recebedor', 'terceiro'].map((r) => (<div key={r} className="flex items-center gap-2">
                      <RadioGroupItem value={r} id={`taker-${r}`}/>
                      <Label htmlFor={`taker-${r}`} className="capitalize font-normal">{r}</Label>
                    </div>))}
                </RadioGroup>
                {active.takerRole === 'terceiro' && (<div className="grid grid-cols-2 gap-2">
                    <div>
              ...
              <TabsContent value="transporte" className="space-y-3 pt-3">
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <Select value={active.documentType} onValueChange={(v) => patch({ documentType: v })}>
                ...
                <div>
                  ...
                  {!active.driverName && (<Badge variant="secondary" className="ml-2 text-[10px]">Emissão com "."</Badge>)}
                  <Select value={active.driverId || 'none'} onValueChange={(v) => {
            if (v === 'none')
                return patch({ driverId: null, driverName: '', driverCpf: '' });
            ...
            patch({ driverId: v, driverName: d?.name || '', driverCpf: d?.cpf || '' });
                ...
                <div>
                  <Label>Veículo</Label>
                  {!active.vehiclePlate && (<Badge variant="secondary" className="ml-2 text-[10px]">Emissão com "."</Badge>)}
                  <Select value={active.vehicleId || 'none'} onValueChange={(v) => {
            if (v === 'none')
                return patch({ vehicleId: null, vehiclePlate: '' });
              ...
              <TabsContent value="carga" className="space-y-3 pt-3">
                <div className="grid grid-cols-4 gap-2">
                  <div>
                    ...
                    <Input type="number" step="0.01" value={Number(active.freightValue ?? 0).toFixed(2)} onChange={(e) => patch({ freightValue: Math.round(Number(e.target.value) * 100) / 100 })}/>
                  </div>
                  <div>
                    <Label>Valor carga (R$)</Label>
                    <Input type="number" step="0.01" value={active.cargoValue} onChange={(e) => patch({ cargoValue: Number(e.target.value) })}/>
                  </div>
                  <div>
                    <Label>Peso (kg)</Label>
                    <Input type="number" step="0.001" value={active.weightKg} onChange={(e) => patch({ weightKg: Number(e.target.value) })}/>
                  </div>
                  <div>
                    <Label>Pallets</Label>
                    <Input type="number" value={active.palletCount} onChange={(e) => patch({ palletCount: Number(e.target.value) })}/>
                  </div>
                </div>
            ...
            ['fcHelper', 'Ajudante'],
                        ...
                        <Input type="number" step="0.01" value={active[k]} onChange={(e) => patch({ [k]: Number(e.target.value) })}/>
                      </div>))}
                  </div>
                </div>
                {(() => {
            const acc = (active.fcDeliveryFee || 0) + (active.fcOthers || 0) + (active.fcInsurance || 0) +
                (active.fcDispatch || 0) + (active.fcGris || 0) + (active.fcToll || 0) +
                (active.fcTracking || 0) + (active.fcLoading || 0) + (active.fcHelper || 0);
            const fretePeso = active.fcFreightWeight > 0
                ? active.fcFreightWeight
                : active.freightValue || 0;
            const icmsSoma = active.icmsEmbutido === true;
            const freteReceber = fretePeso + acc + (icmsSoma ? active.icmsValor || 0 : 0);
            const money = (n) => `R$ ${Number(n || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
            return (<div className="pt-2 border-t">
                      ...
                      <div className="mt-1 rounded-md border divide-y text-sm">
                        ...
                        {active.fcInsurance > 0 && (<div className="flex justify-between px-3 py-1.5">
                            <span>SEGURO</span>
                ...
                <div>
                  ...
                  <div className="rounded-md border max-h-[200px] overflow-auto text-xs">
                    {active.invoices.map((n) => (<div key={n.id} className="flex justify-between border-b px-2 py-1 last:border-0">
                ...
                <div className="pt-2 border-t">
                  ...
                  <div className="grid grid-cols-6 gap-2 pt-1 items-end">
                    <div>
                      ...
                      <select className="flex h-9 w-full rounded-md border border-input bg-background px-2 text-xs" value={active.icmsCst} onChange={(e) => {
            ...
            patch(patchData);
                    ...
                    <label className="flex items-center gap-1 text-xs">
                      <input type="checkbox" checked={active.icmsEmbutido} onChange={(e) => {
            const regime = emitterForActive?.regime_tributario;
            const isSimples = regime === 'simples' || regime === 'mei';
            ...
            patch({ icmsEmbutido: embutido, icmsBase: r.base, icmsValor: r.valor, icmsAliquota: aliq });
                    ...
                    <label className="flex items-center gap-1 text-xs">
                      <input type="checkbox" checked={active.icmsIsento} onChange={(e) => {
            const regime = emitterForActive?.regime_tributario;
            const isSimples = regime === 'simples' || regime === 'mei';
            ...
            patch({ icmsIsento: isento, icmsBase: r.base, icmsValor: r.valor, icmsAliquota: aliq });
                    ...
                    <div>
                      ...
                      <Input type="number" step="0.01" value={active.icmsAliquota} onChange={(e) => {
            const regime = emitterForActive?.regime_tributario;
            const isSimples = regime === 'simples' || regime === 'mei';
            ...
            patch({
                ...
                _aliqManual: true, // Marca que foi alterado manualmente para parar a sugestão
                    ...
                    <div>
                      ...
                      <Input type="number" step="0.01" value={active.icmsBase} onChange={(e) => {
        ...
        <DialogFooter className="flex-wrap gap-2">
          <div className="mr-auto flex items-center gap-2">
            <Button variant="outline" size="sm" disabled={activeIdx === 0} onClick={() => setActiveIdx((i) => Math.max(0, i - 1))}>
              <ChevronLeft className="h-4 w-4"/> Anterior
            </Button>
            <Button variant="outline" size="sm" disabled={activeIdx >= items.length - 1} onClick={() => setActiveIdx((i) => Math.min(items.length - 1, i + 1))}>
              Próximo <ChevronRight className="h-4 w-4"/>
            </Button>
          </div>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Fechar</Button>
          <Button disabled={!allValid || transmitting} onClick={handleTransmitClick}>
            {transmitting ? (<><RotateCw className="h-4 w-4 mr-2 animate-spin"/> Transmitindo…</>) : (<><Send className="h-4 w-4 mr-2"/> Transmitir {items.length} CT-e(s)</>)}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>);
