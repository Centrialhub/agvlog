...
export default function NFSeFromInvoicesDialog({ open, onOpenChange }: Props) {
  ...
  const tomador = useMemo(() => {
    ...
    return {
      ...
      endereco: (match?.address_street || first.recipient_address || first.remitter_address || '') as string,
      numero: ((match as any)?.address_number || first.recipient_number || first.remitter_number || '') as string,
      complemento: ((match as any)?.address_complement || first.recipient_complement || first.remitter_complement || '') as string,
      bairro: (match?.address_neighborhood || first.recipient_neighborhood || first.remitter_neighborhood || '') as string,
      email: ((match as any)?.email || first.recipient_email || first.remitter_email || '') as string,
      telefone: normalizePhone((match as any)?.phone || first.recipient_phone || first.remitter_phone) || '',
