...
import {linkSettlementMovement,readSettlementMovementHistory,readSettlementMovements,SettlementMovementRejectedError} from '@/lib/financial/settlementMovementClient';
...
export function SettlementMovementWorkspace({tenant,actor,payment,settlement,onClose}:{tenant:string;actor:string;payment:string;settlement:string;onClose:()=>void}){
 ...
 const [recoveryError,setRecoveryError]=useState(restored.error||''),[error,setError]=useState(restored.error||''),[choice,setChoice]=useState(''),[reason,setReason]=useState('');
 const [page,setPage]=useState(1),[historyPage,setHistoryPage]=useState(1),[busy,setBusy]=useState(false),[success,setSuccess]=useState(false);
 ...
 const historyQuery=useQuery({queryKey:['finance-settlement-movement-history',tenant,actor,payment,historyPage],queryFn:()=>readSettlementMovementHistory(tenant,payment,historyPage),retry:false});
 const data=query.error?undefined:query.data,contextValid=data?.settlement_id===settlement;
 const history=historyQuery.error?undefined:historyQuery.data,historyContextValid=history?.settlement_id===settlement;
 async function submit(){
  if(sending.current||recoveryError||reversalPending)return;
 ...
 {recoveryError?<div role="alert" className="space-y-2"><p>{recoveryError}</p><Button variant="outline" disabled={busy} onClick={()=>{try{sessionStorage.removeItem(key);setCommand(undefined);setChoice('');setRecoveryError('');setError('');}catch{setError('Não foi possível descartar a recuperação incompatível.');}}}>Descartar recuperação incompatível</Button></div>:error&&<p role="alert">{error}</p>}
 ...
 <section aria-label="Histórico permanente dos vínculos" className="space-y-2"><h3>Histórico de intervenções manuais</h3>
 {historyQuery.isPending&&<p>Consultando histórico…</p>}{historyQuery.isError&&<p role="alert">Não foi possível consultar o histórico. <Button onClick={()=>void historyQuery.refetch()}>Tentar histórico novamente</Button></p>}
 {history&&!historyContextValid&&<p role="alert">Histórico fora do acerto selecionado.</p>}
 {history&&historyContextValid&&<>{history.rows.length===0?<p>Nenhum vínculo nesta página.</p>:history.rows.map(item=><article key={item.id} className="rounded border border-amber-600 p-3"><p>Vínculo manual · {item.reversal?'Desfeito':'Ativo'} · {money(item.amount_cents)}</p><p>Saída: {item.movement_id}</p><p>Vinculado por {item.actor_name} ({item.created_by}) em {new Date(item.created_at).toLocaleString('pt-BR')}</p><p>Motivo: {item.reason||'Consultar auditoria'}</p>{item.reversal&&<><p>Desfeito por {item.reversal.actor_name} ({item.reversal.actor_id}) em {new Date(item.reversal.created_at).toLocaleString('pt-BR')}</p><p>Motivo da correção: {item.reversal.reason}</p></>}</article>)}<div className="flex gap-2"><Button variant="outline" disabled={historyPage===1} onClick={()=>setHistoryPage(historyPage-1)}>Histórico anterior</Button><span>Página {historyPage} do histórico</span><Button variant="outline" disabled={!history.has_more} onClick={()=>setHistoryPage(historyPage+1)}>Próximo histórico</Button></div></>}
 </section>
 <SettlementLinkReversal tenant={tenant} actor={actor} payment={payment} activeLink={contextValid?data?.link?.id||null:null} disabled={!!command||busy||!!recoveryError} onPendingChange={setReversalPending} onConfirmed={()=>{setSuccess(false);setChoice('');}}/>
 {command&&!success?<><p>Pedido preservado: {command.request_id}</p><p>Saída: {command.movement_id}</p><p>Justificativa: {command.reason}</p><Button disabled={busy||!!recoveryError} onClick={()=>void submit()}>{busy?'Confirmando…':'Retomar pedido original'}</Button></>:!success&&contextValid&&!data?.link&&<>
 <fieldset disabled={busy||!!recoveryError}><legend>Saídas com capacidade para o valor integral</legend>{data?.rows.map(row=><label key={row.id} className="flex gap-2 rounded border p-2 my-2"><input type="radio" name="settlement-movement" checked={choice===row.id} onChange={()=>setChoice(row.id)}/><span>{row.description} · {row.account_name} · {row.occurred_on}<br/>{row.beneficiary_name} · total {money(row.amount_cents)} · disponível {money(row.remaining_cents)}</span></label>)}</fieldset>
 ...
 <label>Justificativa<Textarea disabled={!!recoveryError} value={reason} onChange={event=>setReason(event.target.value)} maxLength={2000}/></label>
 <p>Confirme que o valor integral deste pagamento pertence à saída escolhida.</p><Button disabled={!choice||reason.trim().length<10||busy||!!recoveryError||reversalPending} onClick={()=>void submit()}>Confirmar vínculo existente</Button>
 </>}
