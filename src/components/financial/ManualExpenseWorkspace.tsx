...
export function ManualExpenseWorkspace({tenant,actor,onClose}:{tenant:string;actor:string;onClose:()=>void}){
 ...
 const input=(field:keyof typeof form,label:string,type='text')=><label className={field==='description'||field==='notes'||field==='reason'?"block text-sm font-medium sm:col-span-2":"block text-sm font-medium"}>{label}<Input value={form[field]} type={type} onChange={e=>setForm({...form,[field]:e.target.value})}/></label>;
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&!busy){void discardPreview().finally(onClose);}}}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl" onInteractOutside={e=>{if(busy)e.preventDefault();}}><DialogHeader><DialogTitle>Nova despesa avulsa</DialogTitle><DialogDescription>Registre a obrigação e, se já estiver paga, vincule a saída existente. Nenhum pagamento será executado.</DialogDescription></DialogHeader>
  {frozen?<section className="space-y-3"><p className="font-semibold">{frozen.command.description} · {formatFinanceCents(frozen.command.amount_cents)}</p><p>{frozen.command.supplier_name}</p>
  ...
  </section>:<fieldset disabled={busy||!!recoveryError} className="grid gap-4 sm:grid-cols-2">
   ...
   <label className="flex items-center gap-2 rounded-lg border bg-muted/30 p-3 text-sm sm:col-span-2"><input type="checkbox" checked={paid} onChange={e=>setPaid(e.target.checked)}/> Já foi paga com uma saída registrada</label>
   {paid&&<><div className="min-w-0 sm:col-span-2"><ManualExpenseMovementPicker tenant={tenant} actor={actor} value={choice} onSelect={setChoice}/></div><label className="block">Forma registrada<select className="block h-10 w-full rounded border bg-background" value={form.method} onChange={e=>setForm({...form,method:e.target.value})}>{PAYMENT_METHODS.map(m=><option key={m} value={m}>{PAYMENT_METHOD_LABELS[m]}</option>)}</select></label></>}
   <label className="block text-sm font-medium sm:col-span-2">Comprovante (opcional)<Input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/></label>
