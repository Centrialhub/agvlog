...
export function SettlementAdjustmentRecovery({api}:{api:ReturnType<typeof useSettlementAdjustment>}){
 ...
 return <section aria-label="Recuperação de ajustes do acerto" className="mb-4 space-y-2 rounded border p-3">
  {api.recoveryError?<><p role="alert">{api.recoveryError}</p><Button disabled={api.isPending} variant="outline" onClick={async()=>{setNotice({scope,message:''});try{await api.abandon();setNotice({scope,message:'Recuperação incompatível descartada. Você já pode iniciar um novo ajuste.'});}catch(cause){setNotice({scope,message:settlementAdjustmentError(cause)});}}}>Descartar recuperação incompatível</Button></>:null}
