...
export function SettlementPaymentWorkspace({tenant,actor,settlement,initialAmount,allowNew=true,onClose}:{tenant:string;actor:string;settlement:string;initialAmount:number;allowNew?:boolean;onClose:()=>void}){
 ...
 const [recoveryError,setRecoveryError]=useState(restored.error||''),[error,setError]=useState(restored.error||''),[busy,setBusy]=useState(false),[success,setSuccess]=useState(false),[review,setReview]=useState(false);
 ...
 const query=useQuery({queryKey:['finance-settlement-payment-candidates',tenant,actor,settlement,cents,page],queryFn:()=>readSettlementPaymentCandidates(tenant,settlement,cents,page),enabled:allowNew&&cents>0&&!command&&!recoveryError&&!success,retry:false});
 ...
 async function submit(){
  if(sending.current||recoveryError)return;let original=command;
 ...
 {recoveryError?<div role="alert" className="space-y-2"><p>{recoveryError}</p><Button variant="outline" disabled={busy} onClick={()=>{try{sessionStorage.removeItem(key);setCommand(undefined);setReview(false);setChoice('');setRecoveryError('');setError('');}catch{setError('Não foi possível descartar a recuperação incompatível.');}}}>Descartar recuperação incompatível</Button></div>:error?<p role="alert">{error}</p>:null}{!allowNew&&!command&&<p role="alert">Novo registro indisponível nesta abertura. Retome um pedido preservado ou revise o acerto antes de iniciar outro pagamento.</p>}{success?<p role="status">Pagamento registrado e vinculado. Nenhuma nova saída de dinheiro foi criada.</p>:command?<><p>Pedido preservado: {command.request_id}</p><p>Valor: {money(command.amount_cents)} · método: {command.method}</p><p>Saída: {command.movement_id}</p><p>Motivo: {command.reason}</p><Button disabled={busy||!!recoveryError} onClick={()=>void submit()}>Retomar pagamento original</Button></>:<>
 <label>Valor pago (R$)<Input inputMode="decimal" value={amount} disabled={review||!!recoveryError} onChange={event=>{setAmount(event.target.value);setChoice('');setPage(1);}}/></label>
 ...
 <label>Método<select value={method} disabled={review||!!recoveryError} onChange={event=>setMethod(event.target.value as SettlementPaymentCommand['method'])}><option value="pix">PIX</option><option value="ted">TED</option><option value="cash">Dinheiro</option><option value="other">Outro</option></select></label>
 <label>Motivo do registro<Textarea maxLength={2000} disabled={review||!!recoveryError} value={reason} onChange={event=>setReason(event.target.value)}/></label>
 {review?<><p>Confirmar {money(cents)} da saída {selected?.description||choice}. Data do pagamento: {selected?.occurred_on||'Atualizando consulta'}.</p><Button disabled={!valid||busy||!!recoveryError} onClick={()=>void submit()}>Confirmar registro do pagamento</Button><Button variant="outline" disabled={busy} onClick={()=>setReview(false)}>Voltar à edição</Button></>:<Button disabled={!valid||!!recoveryError} onClick={()=>setReview(true)}>Revisar pagamento</Button>}
