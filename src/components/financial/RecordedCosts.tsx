...
export function RecordedCosts({tenant,actor}:{tenant:string;actor:string}){
 ...
 return <section className="space-y-4"><h2 className="text-xl font-semibold">Custos registrados</h2>
  ...
  {data&&!query.error&&<><p className="text-2xl font-semibold">{money(data.total_cents)}</p><p>{data.total} registro(s) no filtro · {data.cancelled_count} cancelado(s), excluído(s) do total</p>
   ...
   <div className="flex flex-wrap gap-2">{data.categories.map(c=><span key={c.category}>{expenseCategories[c.category as keyof typeof expenseCategories]||c.category}: {money(c.amount_cents)}</span>)}</div>
