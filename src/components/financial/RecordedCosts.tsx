...
import {RecordedCostOperationalCoverage} from './RecordedCostOperationalCoverage';
import {LegacyCostInventory} from './LegacyCostInventory';
...
export function RecordedCosts({tenant,actor,onManageCenters}:{tenant:string;actor:string;onManageCenters?:()=>void}){
 ...
 const query=useQuery({queryKey:['finance-recorded-costs',tenant,actor,filters],retry:false,queryFn:()=>readRecordedCosts(tenant,filters)}),data=query.isFetching||query.isError?undefined:query.data;
 const center=(value:string)=>{setFilters({...filters,cost_center:value,page:1});setDraft({...draft,cost_center:value});};
 const category=(value:string)=>{setFilters({...filters,category:value,page:1});setDraft({...draft,category:value});};
 return <section className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold">Custos registrados</h2><div className="flex flex-wrap gap-2">{onManageCenters&&<Button variant="outline" onClick={onManageCenters}>Novo centro de custo</Button>}<Button onClick={()=>setManualExpenseOpen(true)}><Plus className="mr-2 h-4 w-4"/>Adicionar despesa</Button></div></div>
  <p className="text-sm">Gastos em lote e despesas avulsas do novo financeiro. Envios, pagamentos e títulos complementares não são somados novamente. Remuneração da folha aprovada inclui salário, diárias, horas, comissões e bônus. Associações auditadas de manutenção, estoque e despesas antigas identificam representações já existentes sem criar um segundo custo.</p>
  {manualExpenseOpen&&<ManualExpenseWorkspace tenant={tenant} actor={actor} onClose={()=>setManualExpenseOpen(false)}/>}
  ...
  {data&&!query.error&&<><p className="text-2xl font-semibold">{money(data.total_cents)}</p><p>{data.total} registro(s) no filtro · {data.cancelled_count} cancelado(s), excluído(s) do total</p>
   <RecordedCostOperationalCoverage tenant={tenant} actor={actor} from={filters.from} to={filters.to}/>
   {data.needs_review_count>0&&<p role="alert">{data.needs_review_count} despesa(s) precisam de conferência. Valores atuais não comprovados são indeterminados; o original não substitui uma cadeia inválida.</p>}
   {data.recorded_date_count>0&&<p>{data.recorded_date_count} despesa(s) sem competência informada usam a data do registro.</p>}
   {data.payroll_unclassified_count>0&&<p role="alert">No período, {data.payroll_unclassified_count} crédito(s) da folha, somando {money(data.payroll_unclassified_cents)}, aguardam classificação por origem e não entram neste total. Essa pendência considera todos os centros e categorias do período.</p>}
   {filters.cost_center&&<Button variant="outline" onClick={()=>center('')}>Todos os centros</Button>}
   <div className="flex flex-wrap gap-2">{data.cost_centers.map(c=><Button key={c.cost_center_id||'unassigned'} variant="outline" onClick={()=>center(c.cost_center_id||'unassigned')}>{c.cost_center_name||'Sem centro de custo'}: {money(c.amount_cents)}</Button>)}</div>
   {filters.category&&<Button variant="outline" onClick={()=>category('')}>Todas as categorias</Button>}
   <div className="flex flex-wrap gap-2">{data.categories.map(c=><Button key={c.category} type="button" variant={filters.category===c.category?'default':'outline'} onClick={()=>category(c.category)}>{expenseCategories[c.category as keyof typeof expenseCategories]||c.category}: {money(c.amount_cents)}</Button>)}</div>
   <Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Gasto / fornecedor</TableHead><TableHead>Origem</TableHead><TableHead>Centro de custo</TableHead><TableHead>Valor vigente do custo</TableHead><TableHead>Situação</TableHead></TableRow></TableHeader><TableBody>
   {data.rows.map(row=><TableRow key={`${row.source}:${row.id}`}><TableCell>{row.occurred_on.split('-').reverse().join('/')}<p className="text-xs">{row.date_basis==='recorded_date'?'Data do registro':row.date_basis==='payroll_period'?'Início da folha':row.date_basis==='competence_date'?'Competência':'Data do gasto'}</p></TableCell><TableCell>{row.description}<p>{row.supplier_name}</p></TableCell><TableCell>{row.source==='manual_expense'?'Despesa avulsa':row.source==='payroll_item'?'Remuneração da folha':'Gasto em lote'}</TableCell><TableCell>{row.cost_center_name||'Não informado'}</TableCell><TableCell>{money(row.amount_cents)}</TableCell><TableCell>{row.cancelled?'Cancelado — excluído do total':row.needs_review?'Valor a conferir':'Incluído'}{row.needs_review&&<p>Custo ou obrigação precisam de conferência</p>}{row.source==='expense_batch'&&<ExpenseCancellationReview tenant={tenant} actor={actor} expenseId={row.id}/>}{row.source==='manual_expense'&&<ManualExpenseCancellationReview tenant={tenant} actor={actor} payableId={row.id}/>}</TableCell></TableRow>)}
   ...
   <LegacyCostInventory tenant={tenant} actor={actor}/>
  </>}
 </section>;
}
