...
function ScopedPanel({tenant,actor,expense}:{tenant:string;actor:string;expense:string}){
 ...
 return <section className="space-y-3 rounded border p-3" aria-label="Comprovantes adicionais do gasto"><h3 className="font-medium">Comprovantes adicionais</h3>
  ...
  {query.isFetching&&<p role="status">Consultando comprovantes…</p>}{query.isError&&<div role="alert"><p>Não foi possível consultar os comprovantes deste gasto.</p><Button type="button" variant="outline" disabled={query.isFetching} onClick={()=>void query.refetch()}>Tentar novamente</Button></div>}
