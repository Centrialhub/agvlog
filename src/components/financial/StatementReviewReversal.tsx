...
export function StatementReviewReversal({tenant,actor,reviewId,onRecorded}:{tenant:string;actor:string;reviewId:string;onRecorded:()=>void}){
  ...
  async function submit(){
    if(sending.current||recoveryError)return;const command=pending||preview;if(!command)return;const wasUncertain=!!pending;
  ...
  return <section aria-label="Reverter revisão manual" className="space-y-3 rounded border border-amber-600 p-3">
    ...
    {frozen?<><p>Motivo: {frozen.reason}</p>{pending&&<p role="status">Pedido preservado. Retome a confirmação com os mesmos dados.</p>}
      <Button disabled={busy||!!recoveryError} onClick={()=>void submit()}>{busy?'Confirmando…':pending?'Retomar mesma reversão':'Confirmar reversão'}</Button>
      ...
      <Button disabled={!!recoveryError} onClick={prepare}>Revisar reversão antes de registrar</Button></>}
    {error&&<p role="alert">{error}</p>}{recoveryError&&<Button type="button" variant="outline" onClick={()=>{sessionStorage.removeItem(key);setRecoveryError('');setError('');setPending(null);setPreview(null);setOpen(false);}}>Descartar recuperação incompatível</Button>}
  </section>;
}
