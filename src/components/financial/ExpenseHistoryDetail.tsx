...
export function ExpenseHistoryDetail({row,actor,onClose}:{row:ExpenseHistoryRow;actor:string;onClose:()=>void}) {
  ...
  </DialogContent></Dialog>{receipt&&row.receipt_path&&<ExpenseReceiptDialog tenantId={row.tenant_id} path={row.receipt_path} onClose={()=>setReceipt(false)}/>}</>;
}
