...
export function CashPeriodEvidenceSummary({snapshot}:{snapshot:Record<string,unknown>}){
 ...
 return <section aria-label="Contagem preservada no fechamento" className="space-y-2">
  ...
  <div className="overflow-x-auto" role="region" aria-label="Denominações da contagem" tabIndex={0}><table className="w-full text-left"><thead><tr><th>Denominação</th><th>Quantidade</th></tr></thead><tbody>{count.counts.map(row=><tr key={row.denomination_cents}><td>{value(String(row.denomination_cents))}</td><td>{row.quantity}</td></tr>)}</tbody></table></div>
