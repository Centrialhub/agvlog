...
export function StatementReconciliation({tenant,actor,statement}:{tenant:string;actor:string;statement:string}){
 ...
 const [reason,setReason]=useState(''),[accountEvidence,setAccountEvidence]=useState(''),[error,setError]=useState(restored.error),[recoveryError,setRecoveryError]=useState(restored.error||''),[notice,setNotice]=useState(''),[busy,setBusy]=useState(false);
 const sending=useRef(false),live=useRef(true);useEffect(()=>{live.current=true;return()=>{live.current=false;};},[]);
 async function prepare(){
  if(sending.current||recoveryError)return;sending.current=true;setBusy(true);setError('');
 ...
 return <section aria-label="Conciliação manual do extrato" className="space-y-3 rounded border border-amber-600 p-4"><h2 className="font-semibold">Conciliação manual</h2>
  ...
  </>:<><div className="grid gap-3 lg:grid-cols-2"><ReconciliationSelection tenant={tenant} actor={actor} statement={statement} kind="movements" selected={movements} onChange={setMovements} disabled={busy||!!recoveryError}/>
   <ReconciliationSelection tenant={tenant} actor={actor} statement={statement} kind="entries" selected={entries} onChange={setEntries} disabled={busy||!!recoveryError}/></div>
