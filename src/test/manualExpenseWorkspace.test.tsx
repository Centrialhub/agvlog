...
it('discards an incompatible saved expense and enables a fresh review',()=>{const key=`finance-manual-expense:${tenant}:${actor}`;sessionStorage.setItem(key,'broken');open();expect(screen.getByRole('button',{name:'Revisar despesa'})).toBeDisabled();fireEvent.click(screen.getByRole('button',{name:'Descartar recuperação incompatível'}));expect(sessionStorage.getItem(key)).toBeNull();expect(screen.getByRole('button',{name:'Revisar despesa'})).toBeEnabled();expect(screen.queryByText(/Não crie outro pedido/)).not.toBeInTheDocument();expect(mock.record).not.toHaveBeenCalled();});
it('retries the movement options lookup without closing the expense dialog',async()=>{mock.options.mockRejectedValueOnce(new Error('offline')).mockResolvedValueOnce({total:1,rows:[choice]});open();fireEvent.click(screen.getByRole('checkbox'));expect(await screen.findByRole('alert')).toHaveTextContent('Não foi possível consultar as saídas');fireEvent.click(screen.getByRole('button',{name:'Tentar novamente'}));expect(await screen.findByRole('radio')).toBeInTheDocument();expect(mock.options).toHaveBeenCalledTimes(2);});
