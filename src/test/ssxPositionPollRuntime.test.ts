...
import { registerSaturationCases } from './helpers/ssxPositionPollSaturationCases';
import {
      ...
      from: (table: string) => {
        ...
        const builder = {
          ...
          then: (resolve: (value: unknown) => unknown) => {
            ...
            if (orderKey) {
              const key = orderKey;
              rows = [...rows].sort((left, right) => String(left[key]).localeCompare(String(right[key])) * (orderAscending ? 1 : -1));
            }
...
describe('SSX poll handler atomic persistence contract', () => {
  ...
  registerSaturationCases(state, request, accountId);
});
