...
async function remove(expected: string = revision, tenant = ids.tenant) {
  return db.query<{ result: { id: string; deleted_revision: string } }>(
    'select public.delete_operational_route_v1($1,$2,$3) result',
    [tenant, routeId, expected],
  );
}
...
describe('operational route compare-and-delete command', () => {
  ...
  beforeEach(async () => {
    await db.exec('reset role;truncate public.operational_routes,public.tenant_memberships;');
