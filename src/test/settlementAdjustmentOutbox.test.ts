...
import {createSettlementAdjustmentOutbox,pendingSettlementAdjustment} from '@/lib/financial/settlementAdjustmentOutbox';
import type {SettlementAdjustmentCommand} from '@/lib/financial/settlementAdjustmentCommands';
...
const input={settlement_id:'de400000-0000-4000-8000-000000000004',expected_revision:'a'.repeat(32),action:'add' as const,item_id:null,nature:'credit' as const,amount_cents:1000,description:'Diária QA',reason:'Despesa conferida'};
const key='agvlog:settlement-adjustment:v1:'+tenant+':'+actor;
const ack=(p:SettlementAdjustmentCommand)=>({version:1,tenant_id:p.tenant_id,actor_id:p.actor_id,request_id:p.request_id,settlement_id:p.settlement_id,action:p.action,item_id:p.action==='remove'?p.item_id:request,confirmed:true,command_id:tenant,revision:'b'.repeat(32)});
function setup(send=vi.fn(async(p:SettlementAdjustmentCommand):Promise<{data:unknown;error:unknown}>=>({data:ack(p),error:null}))){
 const assertContext=vi.fn();return {send,assertContext,outbox:createSettlementAdjustmentOutbox({storage:localStorage,uuid:()=>request,assertContext,changed:()=>{},lock:async(_key,work)=>work(),send})};
}
beforeEach(()=>localStorage.clear());
describe('settlement adjustment durable outbox',()=>{
 it('persists before transmission and clears only matching confirmation',async()=>{
  const {outbox}=setup(vi.fn(async p=>{expect(pendingSettlementAdjustment(localStorage,tenant,actor)?.payload).toEqual(p);return {data:ack(p),error:null};}));await outbox.submit(tenant,actor,input);expect(localStorage.getItem(key)).toBeNull();
 });
 ...
 it('does not overwrite an uncertain review with another decision',async()=>{
  const {outbox,send}=setup();send.mockResolvedValueOnce({data:null,error:{message:'Resposta perdida'}});await expect(outbox.submit(tenant,actor,input)).rejects.toBeTruthy();const stored=localStorage.getItem(key);await expect(outbox.submit(tenant,actor,{...input,action:'remove',item_id:tenant,nature:null,amount_cents:null,description:null})).rejects.toThrow('Recupere');expect(localStorage.getItem(key)).toBe(stored);expect(send).toHaveBeenCalledTimes(1);
 });
