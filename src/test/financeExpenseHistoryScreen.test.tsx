...
const mocks=vi.hoisted(()=>({history:vi.fn(),summary:vi.fn(),role:'operator',access:true}));
...
vi.mock('@/lib/financial/ledgerClient',async original=>({...await original<object>(),readExpenseHistory:mocks.history,readExpenseHistorySummary:mocks.summary}));
...
const nextCursor={occurred_on:'2026-01-01',created_at:'2026-01-02T12:00:00Z',id:row.id};
const summary={total:31,active_count:31,cancelled_count:0,historical_total_cents:'155000',cancelled_total_cents:'0',total_cents:'155000',allocated_cents:'155000',complement_cents:'0',missing_receipt_count:31,
  cost_centers:[{cost_center_id:null,cost_center_name:null,amount_cents:'155000',item_count:31}],categories:[{category:'food',amount_cents:'155000',item_count:31}]};
const result={version:2,tenant_id:tenant,cursor:null,next_cursor:nextCursor,has_more:true,page_size:30,summary,rows:[row]};
...
beforeEach(()=>{vi.clearAllMocks();mocks.role='operator';mocks.access=true;mocks.history.mockResolvedValue(result);mocks.summary.mockResolvedValue(summary);});
describe('recorded expense history screen',()=>{
  ...
  it('uses the verified corrected cost in the list and payable detail while preserving its original',async()=>{
    ...
    mocks.summary.mockResolvedValue({...summary,total_cents:'12000',complement_cents:'12000'});mocks.history.mockResolvedValue({...result,rows:[{...expense,cost_origin:origin,effective_amount_cents:'12000'}]});
  ...
  it('shows unavailable current cost and totals instead of using the original when evidence is missing',async()=>{
    mocks.summary.mockResolvedValue({...summary,total_cents:null,complement_cents:null,cost_needs_review_count:1,categories:[{category:'food',amount_cents:null,item_count:1}],cost_centers:[]});mocks.history.mockResolvedValue({...result,rows:[{...row,cost_origin:null,effective_amount_cents:null}]});
  ...
  it('filters by the server cost center totals and allows returning to all centers',async()=>{
    ...
    await waitFor(()=>expect(mocks.history).toHaveBeenLastCalledWith(tenant,expect.objectContaining({cost_center:'unassigned',cursor:null})));
    ...
    await waitFor(()=>expect(mocks.history).toHaveBeenLastCalledWith(tenant,expect.objectContaining({cost_center:'',cursor:null})));
  });
  ...
  it('applies filters deliberately instead of using a partial local total',async()=>{
    ...
    await waitFor(()=>expect(mocks.history).toHaveBeenLastCalledWith(tenant,expect.objectContaining({search:'almoço',cursor:null})));
  });
  it('uses a stable keyset cursor and keeps the first-page summary on later pages',async()=>{
    const later={...result,cursor:nextCursor,next_cursor:null,has_more:false,summary:null,rows:[{...row,id:crypto.randomUUID(),description:'Gasto da página seguinte'}]};
    mocks.history.mockResolvedValueOnce(result).mockResolvedValueOnce(later);mount();await screen.findByText('Almoço da viagem');
    fireEvent.click(screen.getByRole('button',{name:'Próxima'}));
    await waitFor(()=>expect(mocks.history).toHaveBeenLastCalledWith(tenant,expect.objectContaining({cursor:nextCursor})));
    expect(await screen.findByText('Gasto da página seguinte')).toBeInTheDocument();expect(screen.getAllByText('R$ 1.550,00').length).toBeGreaterThan(0);
    fireEvent.click(screen.getByRole('button',{name:'Anterior'}));
    await waitFor(()=>expect(mocks.history).toHaveBeenLastCalledWith(tenant,expect.objectContaining({cursor:null})));
  });
