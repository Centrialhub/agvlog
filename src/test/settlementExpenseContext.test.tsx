...
const mock=vi.hoisted(()=>({read:vi.fn()}));vi.mock('@/lib/financial/settlementExpenseContextClient',()=>({readSettlementExpenseContext:mock.read,SettlementExpenseSnapshotChangedError:class extends Error{}}));
const tenant=crypto.randomUUID(),actor=crypto.randomUUID(),settlement=crypto.randomUUID(),payable=crypto.randomUUID();
const data={version:1,tenant_id:tenant,settlement_id:settlement,trip_id:crypto.randomUUID(),page:1,page_size:30,revision:'a'.repeat(32),total:31,total_cents:'100000',allocated_cents:'40000',payable_cents:'60000',paid_cents:'10000',outstanding_cents:'50000',needs_review_count:1,rows:[{id:crypto.randomUUID(),batch_id:crypto.randomUUID(),category:'food',description:'Almoço da viagem',occurred_on:'2026-01-01',amount_cents:'30000',allocated_cents:'10000',payable_id:payable,payee_type:'driver',payee_name:'Motorista Paulo',payable_cents:'20000',paid_cents:'10000',outstanding_cents:'10000',payable_status:'approved',needs_review:true}]};
...
it('shows server totals, payable beneficiary and explicit review without creating another reimbursement',async()=>{
 ...
 fireEvent.click(screen.getByRole('button',{name:'Próxima'}));await waitFor(()=>expect(mock.read).toHaveBeenLastCalledWith(tenant,settlement,2,data.revision));
});
...
it('keeps current cost and obligation indeterminate while displaying known allocation separately',async()=>{mock.read.mockResolvedValue({...data,total_cents:null,payable_cents:null,outstanding_cents:null,rows:[{...data.rows[0],amount_cents:null,payable_cents:null,outstanding_cents:null}]});open();await screen.findByText(/custo Indeterminado/);expect(screen.getByText('Vinculado historicamente: R$ 100,00')).toBeInTheDocument();expect(screen.queryByText('R$ 1.000,00')).not.toBeInTheDocument();expect(screen.queryByText('R$ 0,00')).not.toBeInTheDocument();});
