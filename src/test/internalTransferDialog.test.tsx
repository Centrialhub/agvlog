...
it('discards a corrupted transfer recovery and enables a fresh review',()=>{const key=`finance-transfer:${tenant}:${actor}`;sessionStorage.setItem(key,'broken');open();expect(screen.getByRole('button',{name:'Revisar transferência'})).toBeDisabled();fireEvent.click(screen.getByRole('button',{name:'Descartar recuperação incompatível'}));expect(sessionStorage.getItem(key)).toBeNull();expect(screen.getByRole('button',{name:'Revisar transferência'})).toBeEnabled();expect(screen.queryByText(/Não envie outro pedido/)).not.toBeInTheDocument();expect(mock.record).not.toHaveBeenCalled();});
