...
const data={version:1,tenant_id:tenant,settlement_id:settlement,trip_id:null,page:1,page_size:30,revision:'a'.repeat(32),total:0,total_cents:'0',allocated_cents:'0',payable_cents:'0',paid_cents:'0',outstanding_cents:'0',needs_review_count:0,rows:[]};
it('validates settlement, tenant, pagination and string money fields',async()=>{
 mock.rpc.mockResolvedValueOnce({data,error:null});await expect(readSettlementExpenseContext(tenant,settlement,1)).resolves.toMatchObject({trip_id:null});expect(mock.rpc).toHaveBeenLastCalledWith('get_finance_settlement_expense_context_v2',{_tenant_id:tenant,_settlement_id:settlement,_page:1,_expected_revision:null});
