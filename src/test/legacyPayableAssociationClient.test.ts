...
it('validates tenant, payment, page and nested identity of the paginated consultation',async()=>{
 const response={version:1,tenant_id:tenant,payment_id:payment,revision:'source-1',page_revision:'a'.repeat(32),page:2,page_size:20,total:0,rows:[],payment:{id:payment,payable_id:result.payable_id,bank_account_id:null,account_name:null,supplier_name:null,paid_on:null,amount_cents:null,bank_transaction_id:null,driver_id:null},eligible:false,issue:'finance_legacy_payment_account_invalid',active_link:null,history_total:0,history:[]};rpc.mockResolvedValue({data:response,error:null});await readLegacyPayableAssociation(tenant,payment,2,response.page_revision);expect(rpc).toHaveBeenCalledWith('get_finance_legacy_payable_association',{_tenant_id:tenant,_payment_id:payment,_page:2,_expected_revision:response.page_revision});
 for(const override of [{tenant_id:crypto.randomUUID()},{page:1},{payment:{...response.payment,id:crypto.randomUUID()}}]){rpc.mockResolvedValue({data:{...response,...override},error:null});await expect(readLegacyPayableAssociation(tenant,payment,2,response.page_revision)).rejects.toThrow();}
});
