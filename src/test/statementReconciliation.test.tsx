...
beforeEach(()=>{vi.clearAllMocks();Object.defineProperty(navigator,'locks',{configurable:true,value:{request:(_key:string,work:()=>Promise<unknown>)=>work()}});sessionStorage.clear();localStorage.clear();
...
describe('manual statement reconciliation',()=>{
 it('requires an explicit review and preserves the request before sending',async()=>{
  mount();await review();expect(mock.reconcile).not.toHaveBeenCalled();expect(localStorage.getItem(key)).toBeNull();
  mock.reconcile.mockImplementationOnce(async command=>{expect(JSON.parse(localStorage.getItem(key)!).command.request_id).toBe(command.request_id);return {amount_cents:'50000'};});
  ...
  expect(mock.context).toHaveBeenCalledWith(tenant,[movement],[entry]);expect(mock.reconcile).toHaveBeenCalledTimes(1);expect(localStorage.getItem(key)).toBeNull();
 });
 it('resumes an uncertain confirmation after remount with the same request and frozen selection',async()=>{
  ...
  const command=JSON.parse(localStorage.getItem(key)!).command;first.unmount();mock.options.mockClear();mount();
  ...
  expect(mock.reconcile.mock.calls.map(([sent])=>sent)).toEqual([command,command]);expect(localStorage.getItem(key)).toBeNull();
 });
...
it('unlocks first inactive-movement rejection but preserves a previously uncertain concurrency retry',async()=>{mock.reconcile.mockRejectedValueOnce(new FinanceRejectedError('finance_reconciliation_movement_inactive'));const first=mount();await review();fireEvent.click(screen.getByRole('button',{name:'Confirmar conciliação manual'}));await screen.findByText(/Um movimento selecionado foi invalidado/);expect(localStorage.getItem(key)).toBeNull();expect(screen.queryByRole('button',{name:'Retomar mesma conciliação'})).not.toBeInTheDocument();first.unmount();mock.reconcile.mockRejectedValueOnce(new Error('timeout')).mockRejectedValueOnce(new FinanceRejectedError('finance_dependency_busy'));mount();await review();fireEvent.click(screen.getByRole('button',{name:'Confirmar conciliação manual'}));await screen.findByRole('button',{name:'Retomar mesma conciliação'});const saved=localStorage.getItem(key);fireEvent.click(screen.getByRole('button',{name:'Retomar mesma conciliação'}));await screen.findByText(/Outra operação está alterando uma dependência desta conciliação/);expect(localStorage.getItem(key)).toBe(saved);expect(screen.getByRole('button',{name:'Retomar mesma conciliação'})).toBeInTheDocument();});
