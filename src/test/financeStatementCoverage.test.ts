...
beforeAll(async()=>{
 ...
 for(const name of ['20260909222851_finance_statement_intake','20260909223737_finance_statement_source_verification','20260909230507_finance_statement_queries','20260909231643_finance_statement_identity_review','20260909233625_finance_audit_queries','20260910020543_finance_ofx_statement_intake','20260910021404_finance_native_statement_account','20260910130956_finance_statement_period_evidence','20260910142143_finance_statement_coverage_approvals','20260917080123_compact_statement_coverage_review'])await db.exec(readFileSync(`supabase/migrations/${name}.sql`,'utf8'));
},30000);
...
it('returns compact dependency fingerprints and pages approval history without embedded raw evidence',async()=>{
 await ready();await db.query("select set_config('request.jwt.claim.sub',$1,false)",[i.operator]);const snapshot=(await db.query<{v:{revision:string;dependencies:Record<string,{count:number;revision:string}>;evidence:Record<string,unknown>}}>("select finance_private.statement_coverage_snapshot($1,$2,'2026-08-01','2026-08-31') v",[i.tenant,i.account])).rows[0].v;expect(snapshot.dependencies.imports).toMatchObject({count:2,revision:expect.stringMatching(/^[a-f0-9]{32}$/)});expect(snapshot.evidence).not.toHaveProperty('source_evidence');
 for(let n=0;n<21;n++){const approval=randomUUID();await db.query("insert into finance_statement_coverage_approvals(id,tenant_id,bank_account_id,period_start,period_end,actor_id,actor_name,reason,revision,snapshot,declarations,created_at) values($1,$2,$3,'2026-08-01','2026-08-31',$4,'QA','Cobertura histórica conferida',$5,$6,'{\"originals_obtained_from_bank\":true,\"complete_period_confirmed\":true}',clock_timestamp()+$7*interval '1 second')",[approval,i.tenant,i.account,i.operator,snapshot.revision,snapshot,n]);await db.query("insert into finance_statement_coverage_reversals(tenant_id,approval_id,actor_id,actor_name,reason) values($1,$2,$3,'QA','Correção histórica conferida')",[i.tenant,approval,i.operator]);}
 const first=await read();expect(first.history).toHaveLength(20);expect(first).toMatchObject({history_has_more:true});const second=(await financeAs<{v:{page:number;has_more:boolean;rows:unknown[]}}>(db,i.operator,"select get_finance_statement_coverage_history($1,$2,'2026-08-01','2026-08-31',2) v",[i.tenant,i.account])).rows[0].v;expect(second).toMatchObject({page:2,has_more:false});expect(second.rows).toHaveLength(1);
});
