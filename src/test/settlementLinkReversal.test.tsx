...
it('discards an incompatible saved reversal before enabling correction',()=>{sessionStorage.setItem(key,'invalid');open();expect(screen.getByRole('button',{name:'Corrigir vínculo incorreto'})).toBeDisabled();expect(screen.getByRole('alert')).toHaveTextContent('Não foi possível recuperar');fireEvent.click(screen.getByRole('button',{name:'Descartar recuperação incompatível'}));expect(sessionStorage.getItem(key)).toBeNull();expect(screen.getByRole('button',{name:'Corrigir vínculo incorreto'})).toBeEnabled();expect(mock.reverse).not.toHaveBeenCalled();});
