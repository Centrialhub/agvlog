...
it('retries a failed receipt lookup before enabling a new attachment',async()=>{mocks.read.mockRejectedValueOnce(new Error('offline')).mockResolvedValueOnce({receipts:[]});mount();expect(await screen.findByRole('alert')).toHaveTextContent('Não foi possível consultar');expect(screen.getByLabelText('Arquivo para este gasto')).toBeDisabled();fireEvent.click(screen.getByRole('button',{name:'Tentar novamente'}));await screen.findByText('Nenhum comprovante adicional anexado.');expect(screen.getByLabelText('Arquivo para este gasto')).toBeEnabled();expect(mocks.read).toHaveBeenCalledTimes(2);});
