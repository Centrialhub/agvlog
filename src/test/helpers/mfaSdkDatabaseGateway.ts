...
interface Claims {sub:string;aal:string;exp:number;nonce:number;active_tenant_id?:string}
...
export function mfaSdkDatabaseGateway(db:PGlite){
 const tokens=new Map<string,Claims>(),refreshTokens=new Map<string,{actor:string;aal:string}>(),factors=new Map<string,Factor[]>();
 const selectedTenants=new Map<string,string>();
 ...
 const canAccessTenant=async(actor:string,tenant:string)=>(await db.query<{allowed:boolean}>(`select
   exists(select 1 from public.tenant_memberships where user_id=$1 and tenant_id=$2 and active)
   or exists(select 1 from public.client_portal_access where user_id=$1 and tenant_id=$2 and active) allowed`,[actor,tenant])).rows[0]?.allowed===true;
 const activeTenant=async(actor:string)=>{
  const selected=selectedTenants.get(actor);
  if(selected&&await canAccessTenant(actor,selected))return selected;
  if(selected)selectedTenants.delete(actor);
  return (await db.query<{tenant_id:string}>(`select tenant_id from (
    select tenant_id from public.tenant_memberships where user_id=$1 and active
    union select tenant_id from public.client_portal_access where user_id=$1 and active
   ) authorized order by tenant_id limit 1`,[actor])).rows[0]?.tenant_id;
 };
 const issue=async(actor:string,aal:string)=>{
  const tenant=await activeTenant(actor);
  const claims:Claims={sub:actor,aal,exp:Math.floor(Date.now()/1000)+3600,nonce:++sequence,
   ...(tenant?{active_tenant_id:tenant}:{})};
  ...
  const refreshToken=`synthetic-refresh-${claims.nonce}`;refreshTokens.set(refreshToken,{actor,aal});
  return {access_token:token,refresh_token:refreshToken,expires_in:3600,expires_at:claims.exp,token_type:'bearer' as const,user:getUser(actor)};
 };
 const fetcher:typeof fetch=async(input,init)=>{
  ...
  if(path==='/auth/v1/token'&&url.searchParams.get('grant_type')==='password')return faults.rejectPassword?json({code:'invalid_credentials',message:'Credenciais inválidas'},400):json(await issue(faults.passwordActor,'aal1'));
  if(path==='/auth/v1/token'&&url.searchParams.get('grant_type')==='refresh_token'){
   const previous=refreshTokens.get(String(body.refresh_token||''));
   return previous?json(await issue(previous.actor,previous.aal)):json({code:'refresh_token_not_found',message:'Invalid refresh token'},401);
  }
  ...
  if(factorPath){
   ...
   if(factorPath[2]==='verify'&&method==='POST'){
    ...
    target.status='verified';const next=await issue(claims.sub,'aal2');
  ...
  if(path.startsWith('/rest/v1/rpc/')&&method==='POST'){
   const request=transport.then(async()=>{
    if(init?.signal?.aborted)throw new DOMException('Aborted','AbortError');
    try{
     await adjustmentActor(db,claims.sub,claims.aal,{role:'authenticated',active_tenant_id:claims.active_tenant_id,
     ...
     else if(name==='set_active_tenant_context_v1'){
      const tenant=String(body._tenant_id||'');
      if(!await canAccessTenant(claims.sub,tenant))throw new Error('tenant_context_not_authorized');
      selectedTenants.set(claims.sub,tenant);data={version:1,tenant_id:tenant,workspace_id:null};
     }
 ...
 return {client,newClient,storageKey,stored,requests,faults,factors,drain:()=>transport,signIn:async(actor=i.operator,aal='aal1')=>{
  const session=await issue(actor,aal);const result=await client.auth.setSession(session);if(result.error)throw result.error;return result;
 }};
}
