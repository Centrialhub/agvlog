...
it('discards a corrupted stage recovery and enables a fresh stage',()=>{const key=`finance-transfer-stage:${tenant}:${actor}`;sessionStorage.setItem(key,'broken');open('depart');expect(screen.getByRole('button',{name:'Revisar etapa'})).toBeDisabled();fireEvent.click(screen.getByRole('button',{name:'Descartar recuperação incompatível'}));expect(sessionStorage.getItem(key)).toBeNull();expect(screen.getByRole('button',{name:'Revisar etapa'})).toBeEnabled();expect(screen.queryByText(/Não envie outro pedido/)).not.toBeInTheDocument();expect(mock.record).not.toHaveBeenCalled();});
