...
import {
  applyLoadCommand, createLoadAggregateDatabase, loadAggregateIds as i,
  loadCommand, seedLoad,
...
describe('atomic operator load aggregate commands', () => {
  ...
  it('revalidates membership before replay and exposes only the canonical API', async () => {
    ...
    const acl = (await db.query<Record<string, boolean>>(`select
      ...
      has_table_privilege('authenticated','private.load_aggregate_commands','select') ledger`)).rows[0];
    expect(acl).toEqual({ api: true, anon_api: false, helper: false, ledger: false });
