...
vi.mock('@/lib/financial/ledgerClient',()=>({readFiscalWorkQueue:mock.read}));
function mount(){return render(<QueryClientProvider client={new QueryClient()}><MemoryRouter><FinanceFiscalQueue/></MemoryRouter></QueryClientProvider>);}
const nextCursor={observed_order:'31',observation_id:crypto.randomUUID()};
beforeEach(()=>{vi.clearAllMocks();mock.role='operator';mock.access=true;mock.read.mockResolvedValue({version:2,tenant_id:tenant,page_size:30,status_filter:'review',total:31,cursor:null,next_cursor:nextCursor,has_more:true,
...
describe('fiscal receivables monitoring',()=>{
 it('shows processing failures without presenting them as receipts and resets pagination on filtering',async()=>{
  ...
  fireEvent.click(screen.getByRole('button',{name:'Próxima'}));await waitFor(()=>expect(mock.read).toHaveBeenLastCalledWith(tenant,'review',nextCursor,true));
  fireEvent.change(screen.getByLabelText('Situação'),{target:{value:'pending'}});await waitFor(()=>expect(mock.read).toHaveBeenLastCalledWith(tenant,'pending',null,true));
 });
