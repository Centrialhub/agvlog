...
export function computeSpecificity(table, input) {
    let score = 0;
    const matched = {};
    const ignored = [];
    const check = (field, tableVal, inputVal) => {
        if (!tableVal)
            return; // wildcard
    ...
    const checkSoft = (field, tableVal, inputVal) => {
        if (!tableVal)
            return;
        if (!inputVal) {
    ...
    const checkContains = (field, tableVal, inputVal) => {
        if (!tableVal)
            return;
...
function computeFreightValue(table, input) {
...
export async function calculateFreight(input) {
    ...
    if (error)
        return { success: false, value: 0, breakdown: null, error: error.message };
    if (!tables || tables.length === 0) {
        return { success: false, value: 0, breakdown: null, error: 'Nenhuma tabela de frete ativa encontrada' };
    }
    // Filter by validity
    const valid = tables.filter((t) => !t.valid_until || t.valid_until >= today);
    if (valid.length === 0) {
    ...
    if (qualified.length > 0) {
        ...
        const winnerHasAnyCriteria = !!chosen.table.payer_group || !!chosen.table.payer ||
...
export async function logFreightCalculation(tenantId, entityId, entityType, breakdown, userId) {
