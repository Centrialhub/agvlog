...
import { createReplanningOutbox, pendingReplannings, REPLANNING_CHANGED } from '@/lib/loads/replanningOutbox';
import { isRecord, type ReplanningPayload, type ReplanningResult } from '@/lib/loads/replanning';

export function useLoadReplanning() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('storage', refresh); window.addEventListener(REPLANNING_CHANGED, refresh);
    return () => { window.removeEventListener('storage', refresh); window.removeEventListener(REPLANNING_CHANGED, refresh); };
  }, []);
  ...
  const outbox = useMemo(() => createReplanningOutbox({
    get storage() { return window.localStorage; }, uuid: () => crypto.randomUUID(), assertContext,
    changed: () => window.dispatchEvent(new Event(REPLANNING_CHANGED)),
    lock: async (key, work) => {
  ...
  const pending = useMemo(() => {
    try { return { items: tenant && actor ? pendingReplannings(window.localStorage, tenant, actor) : [], error: null }; }
    catch (error) { return { items: [], error: error instanceof Error ? error.message : 'Falha na recuperação local.' }; }
  }, [tenant, actor, revision]);
  const run = useCallback(async (work: () => Promise<ReplanningResult>) => {
    if (!tenant || !actor) throw new Error('Selecione a empresa e entre com uma sessão válida.');
    if (busy.current) throw new Error('Aguarde o replanejamento em andamento.');
    assertContext(); busy.current = true; setPending(true); let result: ReplanningResult;
  ...
  return { isPending, pending: pending.items, recoveryError: pending.error,
    submit: (payload: Omit<ReplanningPayload, 'tenant_id'>) => run(() => outbox.submit(tenant!, actor!, { ...payload, tenant_id: tenant! })),
