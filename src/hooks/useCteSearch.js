...
function mapOutboundStatus(status, sefaz) {
    ...
    if (s.includes('cancel'))
        return 'cancelled';
    if (s.includes('rejeit') || s.includes('erro'))
        return 'sefaz_error';
    if (st === 'authorized')
        return 'processed';
...
export function useCteSearch(filters, opts) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const docNumber = nz(f.docNumber);
            if (docNumber)
                q = q.ilike('cte_number', `%${docNumber}%`);
            const internal = nz(f.internalNumber);
            if (internal)
                q = q.ilike('internal_number', `%${internal}%`);
            const ref = nz(f.referenceNumber);
            if (ref)
                q = q.ilike('reference_number', `%${ref}%`);
            const accessKey = nz(f.accessKey);
            if (accessKey)
                q = q.ilike('access_key', `%${accessKey.replace(/\D/g, '')}%`);
            const series = nz(f.series);
            if (series)
                q = q.eq('cte_series', series);
            const remitter = nz(f.remitter);
            if (remitter)
                q = q.ilike('remitter', `%${remitter}%`);
            const recipient = nz(f.recipient);
            if (recipient)
                q = q.ilike('recipient', `%${recipient}%`);
            const city = nz(f.recipientCity);
            if (city)
                q = q.ilike('recipient_city', `%${city}%`);
            const consignee = nz(f.consignee);
            if (consignee)
                q = q.ilike('consignee', `%${consignee}%`);
            const payer = nz(f.payer);
            if (payer)
                q = q.ilike('payer_name', `%${payer}%`);
            const payerGroup = nz(f.payerGroup);
            if (payerGroup)
                q = q.ilike('payer_group', `%${payerGroup}%`);
            const driver = nz(f.driverName);
            if (driver)
                q = q.ilike('driver_name', `%${driver}%`);
            const plate = nz(f.vehiclePlate);
            if (plate)
                q = q.ilike('vehicle_plate', `%${plate.replace(/\W/g, '')}%`);
            const trailer = nz(f.trailerPlate);
            if (trailer)
                q = q.ilike('trailer_plate', `%${trailer.replace(/\W/g, '')}%`);
            const ins = nz(f.insuranceCompany);
            if (ins)
                q = q.ilike('insurance_company', `%${ins}%`);
            const contract = nz(f.contractNumber);
            if (contract)
                q = q.ilike('contract_number', `%${contract}%`);
            const trip = nz(f.tripNumber);
            if (trip)
                q = q.ilike('trip_number', `%${trip}%`);
            const invoice = nz(f.invoiceNumber);
            if (invoice)
                q = q.ilike('invoice_numbers', `%${invoice}%`);
            const romexp = nz(f.romexpNumber);
            if (romexp)
                q = q.ilike('romexp_number', `%${romexp}%`);
            if (f.issueDateStart)
                q = q.gte('issued_at', f.issueDateStart);
            if (f.issueDateEnd)
                q = q.lte('issued_at', f.issueDateEnd + 'T23:59:59');
            if (f.cteTypes && f.cteTypes.length > 0)
                q = q.in('cte_type', f.cteTypes);
            if (f.statuses && f.statuses.length > 0)
                q = q.in('sefaz_status', f.statuses);
            const v = bool(f.voided);
            if (v !== null)
                q = q.eq('is_voided', v);
            const c = bool(f.closed);
            if (c !== null)
                q = q.eq('is_closed', c);
            const cp = bool(f.compensated);
            if (cp !== null)
                q = q.eq('is_compensated', cp);
            const af = bool(f.autonomousFreight);
            if (af !== null)
                q = q.eq('autonomous_freight', af);
            const cd = bool(f.complementaryDoc);
            if (cd !== null)
                q = q.eq('complementary_doc', cd);
            ...
            const { data: outbound, error: outErr } = await supabase
                ...
                .select('id, invoice_number, invoice_numbers, access_key, sefaz_status, sefaz_message, status, remitter, recipient, recipient_city, recipient_state, freight_value, value, issue_date, created_at, hub_document_id, emission_id, cte_payload')
                .eq('tenant_id', currentTenant.id)
                .is('deleted_at', null)
            ...
            const hubRows = (outbound || [])
                ...
                .filter((r) => {
                if (docNumber && !has(r.cte_number, docNumber))
                    return false;
                if (accessKey && !has(r.access_key, accessKey.replace(/\D/g, '')))
                    return false;
                if (remitter && !has(r.remitter, remitter))
                    return false;
                if (recipient && !has(r.recipient, recipient))
                    return false;
                if (city && !has(r.recipient_city, city))
                    return false;
                if (payer && !has(r.payer_name, payer))
                    return false;
                if (series || internal || ref || consignee || payerGroup || driver || plate || trailer)
                    return false;
                if (ins || contract || trip || romexp)
                    return false;
                if (invoice && !has(r.invoice_numbers, invoice))
                    return false;
                if (f.statuses?.length && !f.statuses.includes(r.sefaz_status))
                    return false;
                if (f.cteTypes?.length && !f.cteTypes.includes('normal'))
                    return false;
                if (f.issueDateStart && (r.issued_at ?? '') < f.issueDateStart)
                    return false;
                if (f.issueDateEnd && (r.issued_at ?? '') > f.issueDateEnd + 'T23:59:59')
                    return false;
                return true;
            });
            ...
            const filtered = all.filter((r) => {
                if (text) {
                    const hit = has(r.cte_number, text) || has(r.access_key, text) || has(r.remitter, text) ||
                ...
                if (f.downloadable === 'yes' && !(r.hub_document_id || r.pdf_url || r.xml_url))
                    return false;
                if (f.downloadable === 'no' && (r.hub_document_id || r.pdf_url || r.xml_url))
                    return false;
                return true;
            });
