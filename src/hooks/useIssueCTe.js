...
import { buildCtePayload } from '@/lib/fiscal/cteBuilder';
...
export function useIssueCTe() {
    ...
    return useMutation({
        mutationFn: async (input) => {
            ...
            if (!input.emitter?.id)
                throw new Error('Emitente não selecionado');
            ...
            const { data: inserted, error: insErr } = await supabase
                ...
                insured_amount: input.insurer?.insured_amount ?? (input.totals.cargo_value || null),
            ...
            if (insErr)
                throw insErr;
            // 2. Emite no Hub
            const emitBody = { ...built.payload, externalId };
            let hubResponse;
            try {
...
export function useSyncCTe() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: async (fiscalDocumentId) => {
            const { data: doc } = await supabase
                ...
                .maybeSingle();
            const anyDoc = doc;
            if (!anyDoc?.hub_document_id)
                throw new Error('CT-e ainda não transmitido ao Hub Fiscal');
            ...
            const update = {
                ...
                status: d.status === 'authorized' ? 'authorized' : d.status === 'rejected' ? 'rejected' : undefined,
            };
            for (const k of Object.keys(update))
...
export function useCancelCTe() {
    ...
    return useMutation({
        mutationFn: async (args) => {
            ...
            const anyDoc = doc;
            if (!anyDoc?.hub_document_id)
                throw new Error('CT-e ainda não transmitido');
            const res = await hubFiscal.cancel(anyDoc.hub_document_id, args.justificativa.trim(), anyDoc.emission_id || undefined, args.fiscalDocumentId);
            if (res?.success === true) {
