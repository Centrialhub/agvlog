-- Forward-only contain: never restores the unsafe legacy APIs.
...
revoke all on function public.apply_driver_settlement_adjustment(jsonb),settlement_adjustment_private.apply(jsonb) from public,anon,authenticated,service_role;
commit;
