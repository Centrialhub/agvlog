...
begin
 ...
 for c in select * from(values
  ...
  ('public.create_driver_expense_command(jsonb)','55d17f0322a521183a1faf66df7555d9',false,'v','["search_path=\"\""]'::jsonb,true,false),
  ('public.get_expense_creation_context(uuid,text,uuid)','f1258010a1eef808edced6c5f06cdca5',false,'v','["search_path=\"\""]'::jsonb,true,false),
  ('public.inspect_expense_receipt_upload(uuid,uuid,uuid,text,uuid,jsonb)','61d7ee970359b177191ac615f3a83b9e',false,'v','["search_path=\"\""]'::jsonb,true,false),
  ('public.recalculate_manual_expense_settlement(uuid,uuid)','c575298e4167f836cf57c2b6b58696f3',false,'v','["search_path=\"\""]'::jsonb,true,false),
  ...
  ('expense_creation_private.inspect_expense_receipt_upload(uuid,uuid,uuid,text,uuid,jsonb)','4bd6c38cc47cccd614606c8b4c74201c',true,'v','["search_path=\"\""]'::jsonb,true,false),
  ('expense_creation_private.create_driver_expense_command(jsonb)','17eeef15acffe3ce088dd58b0c17792c',true,'v','["search_path=\"\""]'::jsonb,true,false),
  ...
  ('expense_creation_private.recalculate_manual_expense_settlement(uuid,uuid)','174f712c435922d9ec5cc703941f2d10',true,'v','["search_path=\"\""]'::jsonb,true,false)
...
END)')
  ...
  if not exists(select 1 from pg_policy policy_row where policy_row.polrelid=to_regclass(c.relation) and policy_row.polname=c.name and not policy_row.polpermissive and policy_row.polcmd='r' and policy_row.polroles=array['authenticated'::regrole::oid] and replace(pg_get_expr(policy_row.polqual,policy_row.polrelid),E'\r\n',E'\n')=replace(c.expression,E'\r\n',E'\n')) then
   raise exception 'Expense MFA release refused: read boundary changed %',c.name;end if;
 end loop;
 execute 'revoke all on function public.create_driver_expense_command(jsonb) from public,anon,authenticated,service_role';
 execute 'revoke all on function public.inspect_expense_receipt_upload(uuid,uuid,uuid,text,uuid,jsonb) from public,anon,authenticated,service_role';
 execute 'revoke all on function public.recalculate_manual_expense_settlement(uuid,uuid) from public,anon,authenticated,service_role';
 execute 'revoke all on function expense_creation_private.create_driver_expense_command(jsonb) from public,anon,authenticated,service_role';
 execute 'revoke all on function expense_creation_private.inspect_expense_receipt_upload(uuid,uuid,uuid,text,uuid,jsonb) from public,anon,authenticated,service_role';
 execute 'revoke all on function expense_creation_private.recalculate_manual_expense_settlement(uuid,uuid) from public,anon,authenticated,service_role';
end;$release$;
commit;
