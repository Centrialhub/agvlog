...
- Geofences de entrega derivam da localização verificada da parada e são somente leitura no cadastro de frota. Geofences de frota são criadas/editadas por `upsert_geofence_v4` e ativadas, desativadas ou removidas por `mutate_fleet_geofence_v1`; usuários autenticados não têm escrita direta na tabela `geofences`.
