...
try {
  ...
  const {runExtinctionNative}=await import('./test-finance-complement-extinction-native-cases.mjs');
  const result=await runExtinctionNative({query,contested,literal,session,finish});console.log(`${result.passed} native cases passed; ${result.findings} functional gaps found.`);if(result.findings)process.exitCode=1;
} catch (error) {
