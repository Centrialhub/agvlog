...
const directory = mkdtempSync(join(cache, 'cost-center-concurrency-'));
...
try {
  ...
  const {runCostCenterNative}=await import('./test-cost-center-native-cases.mjs');
  const result=await runCostCenterNative({query,contested,literal,session,finish});console.log(`${result.passed} native cases passed; ${result.findings} functional gaps found.`);if(result.findings)process.exitCode=1;
} catch (error) {
