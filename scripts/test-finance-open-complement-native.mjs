...
try {
  ...
  const {runOpenComplementNative}=await import('./test-finance-open-complement-native-cases.mjs');
  const result=await runOpenComplementNative({query,contested,literal,session,finish});console.log(`${result.passed} native cases passed; ${result.findings} functional gaps found.`);if(result.findings)process.exitCode=1;
} catch (error) {
