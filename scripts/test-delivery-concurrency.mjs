...
if(process.env.PG_QA_SUITE==='finance-recorded-cost-operational-coverage') {
const {runRecordedCostOperationalCoverageNative}=await import('./test-finance-recorded-cost-operational-coverage-native-cases.mjs');
console.log(await runRecordedCostOperationalCoverageNative({query,literal,session,finish})+' recorded cost operational coverage native tests passed.');
}
...
if(process.env.PG_QA_SUITE==='finance-period-money-package') {const {runPeriodMoneyPackageNative}=await import('./test-finance-period-money-package-native-cases.mjs');console.log(await runPeriodMoneyPackageNative({query,contested,literal,createRoles:true})+' period money package native tests passed.');}
  ...
  if(process.env.PG_QA_SUITE==='finance-employee-advance-candidates'){const {runEmployeeAdvanceCandidatesNative}=await import('./test-finance-employee-advance-candidates-native-cases.mjs');console.log(await runEmployeeAdvanceCandidatesNative({query,contested,literal,session,finish})+' employee advance candidate native tests passed.');}
  if(process.env.PG_QA_SUITE==='finance-payroll-carryover'){const {runPayrollCarryoverNative}=await import('./test-finance-payroll-carryover-native-cases.mjs');console.log(await runPayrollCarryoverNative({query,contested,literal,session,finish})+' payroll carryover native tests passed.');}
  if(process.env.PG_QA_SUITE==='finance-payable-bulk'){const {runPayableBulkNative}=await import('./test-finance-payable-bulk-native-cases.mjs');console.log(await runPayableBulkNative({query,contested,literal,createRoles:true})+' payable bulk native tests passed.');}
  if(process.env.PG_QA_SUITE==='finance-driver-advance-returns'){const {runDriverAdvanceReturnsNative}=await import('./test-finance-driver-advance-returns-native-cases.mjs');console.log(await runDriverAdvanceReturnsNative({query,contested,literal,session,finish})+' driver advance return native tests passed.');}
if(process.env.PG_QA_SUITE==='finance-cost-returns') {const {runCostReturnsNative}=await import('./test-finance-cost-returns-native-cases.mjs');console.log(await runCostReturnsNative({query,contested,literal,session,finish})+' cost return native tests passed.');}
if(process.env.PG_QA_SUITE==='trip-materialized-correction') {const {runTripMaterializedCorrectionNative}=await import('./test-trip-materialized-correction-native-cases.mjs');console.log(await runTripMaterializedCorrectionNative({query,literal,session,finish})+' materialized trip correction native tests passed.');}
if(process.env.PG_QA_SUITE==='finance-receivable-renegotiation') {const {runReceivableRenegotiationNative}=await import('./test-finance-receivable-renegotiation-native-cases.mjs');console.log(await runReceivableRenegotiationNative({query,contested,literal,session,finish})+' receivable renegotiation native tests passed.');}
