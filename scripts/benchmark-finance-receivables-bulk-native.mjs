...
try {
  ...
  const {runCreditNative}=await import('./benchmark-finance-receivables-bulk-native-cases.mjs');
  const result=await runCreditNative({query,contested,literal,session,finish});console.log(`${result.passed} native cases passed; ${result.findings} functional gaps found.`);if(result.findings)process.exitCode=1;
} catch (error) {
