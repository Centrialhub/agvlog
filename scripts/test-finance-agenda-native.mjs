...
try {
  ...
  const {runAgendaNative}=await import('./test-finance-agenda-native-cases.mjs');
  const result=await runAgendaNative({query,contested,literal,session,finish});console.log(`${result.passed} native cases passed; ${result.findings} functional gaps found.`);if(result.findings)process.exitCode=1;
} catch (error) {
